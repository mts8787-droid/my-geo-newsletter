...
### 5.15 차트 분류 카탈로그 (Chart Library) — 통합

**Chart.js 표준 양식 + 본 레포 실제 적용 차트** 통합 분류. 스킬에서 코드로 호출.

#### Chart.js 표준 (15)

| 코드 | 양식 | 룰 위치 | 본 레포 사용 |
|---|---|---|---|
| **L-1** | Basic Line | §5.11 #1 | C-07 svgLine |
| **L-2** | Interpolation (곡선) | §5.11 #2 | 미사용 |
| **L-3** | Multi-axis | §5.11 #3 | 미사용 |
| **L-4** | Point Styling | §5.11 #4 | 베이스라인 시작점만 |
| **L-5** | Segments | §5.11 #5 | C-08 + C-12 (fadeBeforeIdx 베이스라인) |
| **L-6** | Stepped | §5.11 #6 | 미사용 |
| **L-7** | Styling | §5.11 #7 | dashed 베이스라인 |
| **B-1** | Stacked Bar | §5.14 #1 | 미사용 |
| **B-2** | Stacked Groups | §5.14 #2 | 미사용 |
| **M-1** | Combo Bar+Line | §5.12 #2 | C-14 (참고) |
| **M-2** | Stacked Bar+Line | §5.12 #4 | 미사용 |
| **D-1** | Doughnut | §5.12 #3 | 미사용 |
| **R-1** | Radar (skip points) | §5.12 #5 | 미사용 |
| **BU-1** | Bubble (X/Y/r) | §5.12 #1 | 미사용 |
| **T-1** | Tooltip (직교) | §5.13 | 향후 적용 |

#### 본 레포 실제 적용 (6)

Chart.js 표준 외 본 레포 특화 양식. 컴포넌트 카탈로그 (C-XX) 와 매핑.

| 코드 | 양식 | C-XX | 본 레포 사용 |
|---|---|---|---|
| **V-1** | 그룹 막대 (vbar) | C-05 | dashboard / citation vbar (`_citVBar`) |
| **HB-1** | 가로 막대 (Top N) | C-06 | citation 도메인 Top N 랭킹 |
| **BP-1** | 범프 차트 | C-09 | citation 카테고리 / 도메인 월별 랭킹 변화 (`bumpChartSvg`) |
| **MT-1** | 미니 트렌드 바 (메일 호환) | C-10 | 뉴스레터 / 이메일 본문 (SVG 미지원 클라이언트 호환) |
| **D-2** | 파이 (cutout 없음) | C-19 | citation 카테고리 비중 |
| **BU-2** | 4분면 점도표 | C-20 | tracker / KPI 분석 (impact × effort 등) |
