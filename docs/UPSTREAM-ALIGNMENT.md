...
## 요청 6 — `lg-story` / `lifesgood` 영구 수집 제외

**대상** 크롤 URL 목록 (`audit_list_builder.py` / `build_url_csv.py` 등 수집 대상 생성부)

```
제외 패턴:  /lg-story/   ·  /lifesgood/   ·  /lifes-good/
```

**사유**
- 브랜드 스토리·캠페인 페이지로 **GEO KPI 대상이 아니다** (사용자 결정 2026-08-30).
- 페이지타입 분류가 `newsroom` · `press_media` 로 잘못 잡혀 **보도자료 지표를 왜곡**했다.
  예) `lg.com/au/lifesgood/` 가 '뉴스룸/Press' 로 분류돼 해당 버킷 최저 점수의 원인이 됨.
- 8/30 데이터 기준 **137건** (global 34 · es 17 · vn 16 · br 14 · mx 13 · au 12 · de 12 · in 9 · uk 5 · us 4 · ca 1).

**하류 현재 상태** — 집계 단계에서 이미 제외 중 (`EXCLUDED_URL_RE`).
상류 수집 목록에서 빠지면 하류 코드도 삭제 가능.

---

## 요청 7 — `global` 사이트를 정식 수집 대상으로 유지

`global_<date>_run_<id>.json` 은 `lg.com/global/newsroom` 100p 를 담고 있는데,
하류가 파일명 파서에서 국가코드를 `[a-z]{2}` 로 고정해 **통째로 skip** 하고 있었다
(2026-08-30 수정, `[a-z]{2,10}` 으로 확장).

**요청** — `global` 런을 국가 런과 동일한 주기로 계속 수집.
하류는 이를 **`Global-site`** 라는 별도 사이트 분류로 집계한다 (국가 평균과 분리 표기).

**8/30 결과** — lg-story 33건 제외 후 **67p, 79.6점 (전 사이트 중 최고)**.
