...
- **GCP 단일 플랫폼 원칙** — 데이터 창고(BigQuery), 컴퓨트(Cloud Run), 비밀 관리(Secret Manager), 관찰성(Cloud Monitoring)을 GCP로 통일해 운영·감사·과금을 한 곳에서 관리
- **기존 Render는 단계적 이전** — 초기에는 Render를 그대로 두고 브릿지 API로 BigQuery 결과만 받아오다가, 안정화된 뒤 Cloud Run으로 컷오버. 에디터·템플릿 코드는 바꾸지 않음
- **Claude API는 리포트 생성 전용** — Vertex AI는 임베딩·메타 분류에만 쓰고, 본문 생성과 Tool Use는 Claude API로 고정
- **Terraform + GitHub Actions** — 모든 GCP 리소스를 IaC로 선언, OIDC 기반 배포로 서비스 계정 키 파일을 레포에 두지 않음
- **최소 권한 IAM** — 서비스 계정 9개를 역할별로 분리, 각자 필요한 BigQuery 데이터셋·버킷·시크릿만 접근
...
| LLM/엔진 가변비 | 통제 불가 | 월 $400~800 (예산 알림 $1,500 상한) |
| 장애 복구 | 수동 | Monitoring 알림 + 자동 재시도 |
| 감사 이력 | 로그 수준 | BigQuery `audit_logs` 전체 append-only |
