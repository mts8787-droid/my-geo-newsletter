...
## 2. 자동 매칭 원리 — 어떻게 "뉴스레터 만들어줘" 가 `newsletter-make` 를 호출?

### 2.1 Claude 가 항상 보고 있는 것

Claude Code 가 실행되면 시작 시점에 모든 `.claude/skills/*/SKILL.md` 의 **frontmatter 만** 일괄 로드. 본문은 안 읽음 — frontmatter 의 `name` + `description` 으로 "스킬 목록" 만 인지.

```
시스템 메시지에 자동 등재:
- newsletter-make: 뉴스레터 / 월간·주간 리포트 신규 발행본 작성 워크플로우. ...
- newsletter-debug: 뉴스레터 / 이메일 클라이언트 호환 회귀 디버깅 — ...
- data-add: 데이터 모델에 무언가를 신규로 "추가" 할 때 — ...
- ...
```

### 2.2 사용자 발화 → 스킬 매칭

사용자가 "5월호 뉴스레터 만들어줘" 라고 입력하면, Claude 가:
1. 모든 스킬의 `description` 을 자연어로 비교
2. 키워드 / 의도 가장 유사한 스킬 식별 — `newsletter-make` 의 description 에 "5월호" / "뉴스레터" / "신규 발행본" 같은 표현 있음 → 매칭
3. 그 스킬의 SKILL.md **본문을 읽고** step-by-step 따름

매칭이 안 되면? Claude 가 일반 LLM 으로 작업 진행 (스킬 없이). 매칭 가능성 높이려면 description 에 사용자가 자주 쓸 표현 명시.

### 2.3 description 작성 원칙 (매칭이 잘 되려면)

- **트리거 예시 문구 직접 포함**: "사용자가 '○○ 해줘' 같이 요청 시 트리거" 형태
- **언제 사용 X 도 명시**: "신규 추가가 아닌 기존 코드 작업에 트리거" 같이 경계 설정
- **카테고리 분기**: 인덱스 스킬은 "성격별 N skill 로 분리됨" 명시 — 정밀 매칭을 sub-skill 로 유도
- **참조 룰 명시**: "Rule·매뉴얼은 .claude/rules/X.md 참조" — Claude 가 매뉴얼 본문 외 참조처 파악 가능

---

## 3. 슬래시 명령 (`/skills <name>`) vs 자연어 호출
...
### 5.4 스킬이 아예 매칭 안 되는 발화 — frontmatter 갱신 신호
