...
| 영역 | 스킬 | 사용자 발화 → 무엇을 함 |
|---|---|---|
| 🗂 데이터 | `data` · `data-add` · `data-debug` · `data-refactor` | "새 시트 추가" / "데이터 갑자기 이상해" / "파서 정리" 같이 말하면 데이터 통합·원인 찾기·코드 정리 |
| 🎨 디자인 | `design` · `design-layout` · `design-chart` · `design-component` · `design-tune` | "신규 페이지 레이아웃" / "L-1 차트" / "새 카드" / "이메일에서 차트 안 보임" 같이 말하면 시각 작업 |
| 📧 뉴스레터 | `newsletter` · `newsletter-make` · `newsletter-debug` · `newsletter-send` | "5월호 만들어줘" / "Outlook 에서 깨짐" / "발송 검증" 같이 말하면 발행 / 깨진 메일 고치기 / SMTP |
| 🔧 공통·메타 | `onboard` · `debug` · `prompting` | "이 하네스 적용" / "이거 안 됨" / "Cursor 용 프롬프트" 같이 말하면 셋업 / 일반 디버깅 / 다른 도구 통합 |
