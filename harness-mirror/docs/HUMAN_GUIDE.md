...
Claude 한테 **"이 하네스 우리 프로젝트에 적용해줘"** 또는 **"이거 안 되는데 디버깅해줘"** 같이 말하면, Claude 가 내부적으로 준비된 시나리오를 따라가면서 사용자에게 단계별로 질문하고 진행합니다. 본 문서는 **그 시나리오를 사람이 어떻게 호출하고 활용하는지** 설명해요.

> 시나리오 자체 (`.claude/rules/BOOTSTRAP.md`) 는 Claude 가 읽는 1:1 매뉴얼이라 사람이 직접 읽기엔 길어요. 본 문서는 사람용 요약 + 사용법.

---

## 미리 알아둘 것 — 3 가지 시나리오 세트

`.claude/rules/BOOTSTRAP.md` 안에는 세 묶음의 시나리오가 들어 있어요:

1. **신규 프로젝트 적용** (STEP 0~8) — 새 프로젝트에 본 하네스 처음 셋업할 때. Claude 가 도메인·데이터·디자인 토큰 등 질문하면서 셋업.
2. **디버깅** (DEBUG-1 ~ DEBUG-15) — "X 안 되는데" 같은 회귀·오류 보고 시 Claude 가 어떤 패턴으로 진단할지 정리한 카탈로그.
3. **협업·디버깅 기법** (TECHNIQUE-1 ~ TECHNIQUE-23) — Claude 가 정보 부족하거나 막혔을 때 활용하는 23 가지 팁 (콘솔 로그·원본 데이터 공유·픽셀 단위 명령 등).
...
### 트리거
```
사용자 → Claude: "이 하네스 적용해줘. 내 프로젝트는 [프로젝트 종류]"
                  (예: "매출 대시보드", "HR", "마케팅 KPI", "의료")
```
...
| 6. 비즈니스 fact | `DEFAULT_XXX` 객체 정의 | "데이터 무관 Rule" 알려주기 |
| 7. 첫 빌드 + 검증 | `npm test / build / start` | 브라우저로 확인 |
| 8. 반복 / 확장 | 후속 작업 패턴 안내 | 신규 작업 시 해당 Skill 사용 |
...
| **DEBUG-16** | 디자인+데이터 | 필터 작동 안 함 / 필터 후 차트 사라짐·갱신 X |
...
| **TECHNIQUE-8** | 점진적 변경 + 회귀 검증 사이클 | 큰 리팩터링 / 매핑 통합 | Phase 별 변경 → npm test → 커밋 반복 (회귀 시 phase 만 되돌림) |
| **TECHNIQUE-9** | 잔여 패턴 grep 회귀 방지 | single source 통합 후 검증 | `grep -rn "tv: 'TV'" src/` 잔여 0건 확인 |
| **TECHNIQUE-10** | invariant 런타임 + 테스트 자동 검증 | 매핑 / 토큰 동시 갱신 필요 | `assertXxxInvariant()` 모듈 로드 + 테스트 강제 — 일부 누락 자동 차단 |
| **TECHNIQUE-11** | Sub-Agent 도메인 진단 (read-only) | 큰 영역 정합성 점검 | data-puller 같은 read-only agent 위임 — 진단만, 수정은 메인 |
| **TECHNIQUE-12** | 본질 변경 + 산출물 분리 커밋 | dist-* 같은 빌드 산출물 git 추적 | feat 커밋 + chore(dist) 커밋 분리 — git log 가독성 |
| **TECHNIQUE-13** | 페이지 분리 + 통합 관리자 | 디자인 복잡 / 데이터 여러 종류 | SPA 별 분리 + 어드민 메뉴가 통합 진입점 (3단 그리드) |
| **TECHNIQUE-14** | 헤더·필터 조합 꼬임 방지 | 페이지 분리 시 필터 동기화 | state 격리 / data-prodid 매칭 / GNB+필터 레이어 분리 |
| **TECHNIQUE-15** | 데이터 완전 틀어짐 — 원본 헤더 검증 | 모든 차트 빈 화면 / 값 이상 | 원본 헤더 변경 확인 + 빈셀/null 처리 정책 + findHeaderIdx 정규식 확장 |
| **TECHNIQUE-16** | data-prodid 매칭 키 검증 | 필터 후 트렌드 사라짐 | DOM row 의 data-prodid 속성 / dataset.prodid 매칭 |
| **TECHNIQUE-17** | 클라이언트 짝 함수 일치성 | 첫 렌더 OK · 필터 후 깨짐 | 서버 svgLine ↔ 클라 _miniSvg 동시 수정, 옵션 시그니처 일치 |
| **TECHNIQUE-18** | 필터 영향 카드 전체 점검 | 일부 카드만 갱신 | onFilterChange 가 Hero / 카드 / 트렌드 / vbar / 섹션 모두 호출 |
| **TECHNIQUE-19** | null 요소 안전 처리 | 빈 셀 / 미측정 점 잘못 렌더 | _miniSvgNullAware — null 점 미렌더 + 선 끊김 |
| **TECHNIQUE-20** | truncate 인덱스 갱신 | 드롭다운 변경 시 차트 그대로 | _curWeekIdx 갱신 + SVG 옵션 truncateAfter 전달 |
| **TECHNIQUE-21** | 드롭다운 옵션 자체 검증 | TTL 없는 달 선택 불가 | 모든 시점 옵션에 포함 + 빈 데이터 시 명시적 메시지 |
| **TECHNIQUE-22** | Total 폴백 전략 | 국가 데이터 없으면 빈 화면 | 자동 Total 폴백 또는 "데이터 없음" 메시지 |
| **TECHNIQUE-23** | 필터 state 격리 | 페이지 분리 후 필터 누설 | 각 SPA 자체 state, URL/localStorage 외 공유 X, GNB+필터 분리 |
...
### 어드민 UI
- [ ] 메뉴 3단 구조 정상
- [ ] `/admin/harness` — 본 프로젝트 Rule/Hook/Skill 노출
...
| 증상 | 원인 | 해결 |
|---|---|---|
| Claude 가 Rule 안 읽음 | `.claude/` 디렉토리 잘못된 위치 | 프로젝트 루트에 있는지 확인 |
| Hook이 실행 안 됨 | `chmod +x` 안 했음 | `chmod +x .claude/hooks/*.sh` |
...
| 이메일 우측 잘림 | `containerWidth > 940` | 940 이하 (NEVER Rule) |
| iframe 안 차트 클립 | `body { overflow-x:hidden }` 또는 `min-width` | 두 속성 제거 (NEVER Rule) |
...
- `"신규 시트 X 추가해줘"` → 데이터 Skill
- `"신규 카테고리 Y 추가"` → categoryMap 갱신
- `"L-1 차트 그려줘"` 또는 다른 분류 코드 → 디자인 Skill
- `"이 회귀 디버깅 해줘"` → 디버깅 Skill (DEBUG-N 매칭)

### Q2. 본 하네스를 본인 도메인에 추상화하려면?
A. `.claude/skills/onboard/SKILL.md` 의 "도메인 추상화 검토" 워크플로우.

### Q3. 신규 Rule / Hook / Skill 추가?
...
- **Rule 매뉴얼** — `.claude/rules/data.md` · `design.md` · `ai.md`
- **Skill 워크플로우** — `.claude/skills/data/SKILL.md` · `design/SKILL.md` · `onboard/SKILL.md` · `debug/SKILL.md`
- **Hook** — `.claude/hooks/README.md`
