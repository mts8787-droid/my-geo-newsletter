---
name: design-tune
description: 디자인 미세 조정 / 호환성 / 디버깅 워크플로우 — 기존 SVG·flex 컴포넌트를 이메일 호환 table-layout 으로 변환, 다국어 (KO/EN) 라벨 추가, UI 컴포넌트 깨짐·차트 안 나옴·텍스트 클립 회귀 디버깅. "이메일에서 차트 안 보임", "EN 라벨 추가", "카드 깨짐" 같이 호환·미세조정·회귀 시 트리거.
---
