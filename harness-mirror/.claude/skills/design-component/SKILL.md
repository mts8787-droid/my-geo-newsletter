...
description: '시각 컴포넌트 1개 추가 워크플로우 (신규 카드/테이블 C-XX 등재, 뉴스레터 V4 변형, 어드민 iframe 미리보기). 새 시각 단위를 추가할 때 호출 (전체 레이아웃·차트 자체는 별개). 트리거: "새 카드 만들어줘" / "C-24 추가" / "뉴스레터 V4" / "컴포넌트 추가" 같은 요청.'
---
