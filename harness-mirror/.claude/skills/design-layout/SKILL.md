---
name: design-layout
description: '전체 페이지·대시보드 골격 설계 워크플로우 (8 섹션 + 반응형 그리드 + 폴더 구조 — 시트 스키마 우선 참조). 신규 페이지를 처음부터 짤 때 호출 (기존 페이지 조정은 design-tune). 트리거: "대시보드 레이아웃" / "신규 페이지 만들어줘" / "전체 페이지 구성" / "어떻게 배치할까" 같은 요청.'
---
