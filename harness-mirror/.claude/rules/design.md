...
본 Rule 은 디자인 시스템의 **공통 패턴**. 본 저장소는 가전 GEO KPI 대시보드 + 뉴스레터라 색상·컴포넌트·차트가 가전 색깔이지만, **패턴 자체는 같은 가전 산업 내 영업·매출·전략·마케팅·R&D 등 다른 직무 도메인 시각화에도 그대로 적용**.
...
- **영업 대시보드** — `BRAND_COLORS` 대신 `CHANNEL_COLORS` / `STATUS` 그대로 사용
- **마케팅 대시보드** — `BRAND_COLORS` → `CAMPAIGN_COLORS` / 베이스라인 마커는 캠페인 시작 시점 표시
- **전략 보고서** — 차트 분류 코드 (`L-1`, `BP-1` 등) 는 그대로 호출 가능

### 부트스트랩 사용법 (비개발자)

본 Rule 직접 안 봐도 됨. 부트스트랩 시:

1. "회사 색상 / 폰트가 뭐예요?" 만 답
2. 데이터 shape 보여주면 → 🐈‍⬛ 히로가 적절한 차트 분류 코드 자동 매칭
3. 컴포넌트 (`C-01 Hero`, `C-03 카드` 등) 자동 적용
