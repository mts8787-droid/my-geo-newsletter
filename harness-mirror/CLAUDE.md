...
## 장기 호흡 작업 — 컨텍스트 압축 후 계속 진행

대화가 길어져 시스템이 자동 압축 (compact) 해도 **일찍 중단 금지**. 압축 직후 계속 진행하기 위한 외부 상태 보존 패턴:

| 상태 종류 | 보존 위치 | 사용 시점 |
|---|---|---|
| **진행 단계** | TaskCreate / TaskList (대화 내 task 시스템) | 멀티 step 작업 중 — 압축 후에도 task 상태 그대로 |
| **검증 기준** | 영구 파일 — 예: `<work-dir>/progress.json` / `tests.json` | "이 작업이 끝났는지" 외부 판정 필요할 때 (테스트 통과 / 빌드 성공 등) |
| **체크포인트** | `git commit` (작업 단위마다) | 압축 / 컨텍스트 손실 시 마지막 안전 지점 |
| **자유 노트** | 작업 디렉토리의 임시 `.md` (예: `WIP.md`, gitignore) | "지금까지 알아낸 사실" / "다음 시도할 방향" |

**RULE**:
- 큰 작업 (>1시간 / 멀티 파일) 시작 시 → TaskCreate 로 분해
- 각 task 완료 시 즉시 `git commit` (압축 발생해도 작업 진척 보존)
- 압축 후 첫 행동: `TaskList` 로 현재 상태 확인 → `git log -5` 로 최근 진척 확인 → 마지막 task 의 in_progress 부터 재개
- 검증 기준이 복잡하면 (예: 3개 invariant 동시 통과) `WIP.md` 같은 자유 노트로 명시 (대화 압축돼도 파일은 살아남음)
- 사용자가 "작업이 길 것 같은데" / "오래 걸리는 거 시작해" 같이 말하면 TaskCreate + 작은 단위 commit 전략 자동 가동

**ANTI-PATTERN**:
- 작업 중간에 압축 발생 → 작업 끝났다고 보고 (실제는 미완료) — 항상 TaskList / git log 로 확인 후 보고
- 진척을 대화 안에만 기록 (외부 보존 X) → 압축 시 손실
