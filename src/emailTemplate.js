...
const RD_TEXT = {
  ...
  summary: '8월 LG.com의 전체적인 Readability 점수는 75.9점으로 전월 대비 1.6점 하락했으며, 국가별로는 캐나다가 77.6점으로 가장 높았고, 가장 점수가 낮은 인도가 73.4점으로 편차가 크지 않은 상황입니다.\n페이지 타입별로도 PDP 77.8점, 뉴스룸/Press 71.5점으로 유사한 분포를 보이고 있습니다.',
  areaIntro: '반면 평가 영역별로는 점수 차이가 뚜렷하게 나타났습니다.\n사이트 성능 99.0점 (페이지 응답 및 정보 제공 속도), AI Crawlability 96.1점 (AI 크롤러가 원문을 가져갈 수 있는 기술 환경), 웹접근성 88.7점 (사람과 AI가 문서 구조를 읽어낼 수 체계), Basic SEO 85.7점 (제목·페이지 주제 설명 등 검색되기위한 기본 정보)으로 비교적 양호했습니다.\n반면, 스키마마크업 36.0점 (제품·FAQ·이미지 등 정보의 종류와 관계를 알려주는 구조화 데이터), 고인용 콘텐츠 29.1점 (AI가 인용할만한 서술의 존재)은 상대적으로 낮았습니다.',
  d1Title: '사이트 성능과 AI Crawlability는 전반적으로 우수하나, 초기 HTML 내 텍스트 제공 비중 보완 필요',
  d1: '사이트 성능 영역은 99.0점으로 가장 높은 점수를 기록하여 LG.com의 빠르고 안정적인 페이지 전달 수준을 확인할 수 있었습니다.\nAI Crawlability 또한 96.1점으로 상대적으로 높지만, JS HTML Text Ratio(Javascript 렌더링 후 텍스트 대비 초기 HTML Text Count 비중이 60% 이상) 충족률은 78.6%였습니다.\n특히 Support-Trouble Shooting 페이지는 34.5%, PDP는 60.5%로 낮아, 일부 주요 콘텐츠가 페이지를 처음 불러오는 시점에는 포함되지 않고 화면이 열린 후 추가로 불러와지는 방식(CSR)으로 제공되고 있었습니다.\n사용자가 최종 화면에서 콘텐츠를 확인하는 데는 문제가 없을 수 있지만, 페이지를 처음 전달받은 상태를 중심으로 정보를 수집하는 일부 AI는 주요 내용을 충분히 확인하지 못할 가능성이 있습니다.\n때문에 현재 고객가치혁신 및 D2C에서는 Support 페이지와 PDP내 주요 정보가 페이지를 처음 불러오는 시점부터 포함되도록 SSR 방식으로 전환하는 작업을 추진 중입니다.',
  d1Notes: [
  ...
  d2: '웹접근성은 88.7점으로 높았지만, 제목과 소제목이 순서에 맞게 구성됐는지를 확인하는 Heading Hierarchy 충족률은 65.4%로 상대적으로 낮았습니다.\nAI가 페이지의 주제와 세부 내용을 보다 명확하게 구분할 수 있도록, 페이지 유형별 제목 구조를 표준화할 필요가 있습니다.\nBasic SEO 영역 역시 전체 점수는 85.7점으로 양호하지만, 페이지의 제목인 H1이 정확하게 1개만 존재하는 비율은 73.6%, Meta Description 충족률은 79.5%로 개선 여지가 확인됐습니다.\n7월에 지적했던 Newsroom 페이지의 기본 정보 누락은 크게 개선되어 Meta Description 충족률이 34.4%에서 87.7%로, H1 충족률이 46.4%에서 62.0%로 상승했습니다. 페이지 템플릿 차원의 개선이 효과를 낸 것으로 보이며, H1은 추가 보완이 필요합니다.\nSitemap 충족률은 70.1%로 전월(77.6%) 대비 하락해 지속적인 최신화 작업이 필요합니다.',
  d3Title: '스키마마크업은 페이지 타입과 콘텐츠에 맞춘 최적화가 필요',
  d3: '스키마 영역은 36.0점으로 전월(35.5점)과 유사하게 낮은 수준입니다. Schema는 AI가 FAQ·제품·이미지·영상·사용방법 등 정보의 종류를 구분할 수 있게 해주는 정해진 형식으로, 페이지 유형과 콘텐츠 특성에 맞춰 주요 Schema의 적용 범위를 확대할 필요가 있습니다.\n상세 현황 파악을 위해 독일 LG.com과 Samsung.com 확인해보았을 때, PDP의 핵심 스키마인 Product(LG 0%, SS 64%) 스키마는 경쟁사 대비 충족률이 낮았고, PLP의 경우 특히 FAQPage 스키마 적용률 격차가 크게 나타났습니다(LG 13%, SS 71%). 이 외에도 VideoObject, ImageObject, HowTo, Article 모두 주요 스키마로 전 페이지 타입에 걸쳐 개선이 필요한 상황입니다. 이에 D2C에서 추진 중인 Schema 자동화를 통해, 국가와 페이지별로 개별 대응하기 보다 주요 Schema가 일관되게 생성, 적용되는 구조를 마련하는 중입니다.',
  d4Title: '고인용 콘텐츠는 AI 답변에 적합한 콘텐츠 형식 확대와 작성자 정보 보완 필요',
  d4: '콘텐츠 영역은 29.1점으로 전체 평가 영역 중 가장 낮았습니다. AI가 질문에 대한 직접적인 답변으로 활용하기 쉬운 FAQ Block, Summary Box, Definition Paragraph 등의 콘텐츠가 충분하지 않아 보완이 필요한 상황이며, 현재 FAQ Block을 중심으로 각 사업본부 및 고객가치혁신의 주요 개선 과제로 추진 중입니다.\n한편, Citable Sentence(숫자, 연도, 통계, 연구 키워드 포함 문장)의 경우 전체 충족률은 53.6%였습니다. 독일, 스페인, 호주, 캐나다의 경우 충족률 80% 이상인 반면, 나머지 국가에서는 상대적으로 낮게 나타나 국가별 편차가 큰 상황입니다. 7월 독일 사이트를 비교해본 결과, LG.com의 Citable Sentence 충족률은 86.1%로 Samsung.com의 19.4%보다 크게 높아, LG 콘텐츠의 경쟁 우위 요소로 확인되었습니다. 향후 인도, 멕시코, 베트남, 미국 등 상대적으로 충족률이 낮은 국가에서도 Citable Sentence 확대를 통해 경쟁 우위 요소를 강화할 필요가 있습니다.\n콘텐츠의 작성자/출처/날짜 정보 충족률 또한 34.9%로 낮게 나타났습니다. 이는 AI가 콘텐츠의 신뢰성과 최신성을 판단하는 데 참고할 수 있는 요소인 만큼, 정보성 콘텐츠를 중심으로 보완이 필요합니다. 다만 해당 요소의 중요도가 높은 프레스앤미디어의 경우 충족률이 74.3%로 비교적 양호했으며, 구매 가이드와 LG Experience는 0%로 보완이 시급합니다.',
}
...
const RD_TEXT_EN = {
  intro: 'Readability measures how well a web page can be read and used by AI. From July 2026 we began assessing LG.com\u2019s Readability across 5,652 pages spanning the major page types in 10 strategic countries (as of Aug 30). The Readability score is the share of checklist items meeting the bar, normalized to a 100-point scale. The assessment covers 6 areas \u2014 Site Performance, AI Accessibility, Basic SEO, Schema Markup, Citable Content, and AI Crawlability \u2014 across 38 checklist items in total.',
  introNotes: [
  ...
  summary: 'LG.com scored 75.9 overall for Readability in August, down 1.6 points month over month. By country, Canada led at 77.6 and India trailed at 73.4 \u2014 a narrow spread.\nPage types show a similar distribution: PDP 77.8, Newsroom / Press 71.5.',
  areaIntro: 'By assessment area, however, the gaps are pronounced.\nSite Performance 99.0 (page response and content delivery speed), AI Crawlability 96.1 (the technical conditions that let AI crawlers retrieve the source), Accessibility 88.7 (structure that both people and AI can parse), and Basic SEO 85.7 (the baseline information needed to be found \u2014 titles, page descriptions) were relatively strong.\nSchema Markup 36.0 (structured data that signals the type of information and how it relates) and Citable Content 29.1 (the presence of statements AI can quote) were comparatively low.',
  ...
  d1: 'Site Performance scored highest at 99.0, confirming that LG.com delivers pages quickly and reliably.\nAI Crawlability was also relatively high at 96.1, but the JS HTML Text Ratio (initial-HTML text accounting for 60% or more of post-JavaScript text) was met on only 78.6% of pages.\nSupport-Troubleshooting (34.5%) and PDP (60.5%) were especially low, meaning some key content is not present when the page is first delivered and is instead fetched after the view opens (CSR).\nThis may pose no problem for users viewing the final screen, but AI systems that collect information primarily from the initially delivered page may fail to see key content.\nCustomer Value Innovation and D2C are therefore migrating Support pages and key PDP information to SSR so that they are included from the first page load.',
  d1Notes: [
  ...
  d2: 'Accessibility scored a high 88.7, yet Heading Hierarchy \u2014 whether headings and subheadings follow a correct order \u2014 was met on only 65.4% of pages.\nHeading structures should be standardized per page type so AI can more clearly separate a page\u2019s topic from its details.\nBasic SEO likewise scored a sound 85.7 overall, but only 73.6% of pages carry exactly one H1 and 79.5% meet the Meta Description bar \u2014 both leave room to improve.\nThe Newsroom gaps flagged in July improved sharply: Meta Description rose from 34.4% to 87.7% and H1 from 46.4% to 62.0%. The page-template fix appears to be working, though H1 still needs further work.\nSitemap coverage fell to 70.1% from 77.6% and calls for ongoing maintenance.',
  d3Title: 'Schema Markup needs optimization tailored to page type and content',
  d3: 'Schema stayed low at 36.0, little changed from 35.5 in July. Schema is the agreed format that lets AI distinguish types of information \u2014 FAQ, product, image, video, how-to \u2014 so its coverage should be widened to match page type and content characteristics.\nComparing LG.com and Samsung.com in Germany for detail, Product schema \u2014 the core schema for PDPs \u2014 lagged the competitor (LG 0%, SS 64%), and on PLPs the FAQPage adoption gap was especially wide (LG 13%, SS 71%). VideoObject, ImageObject, HowTo, and Article are all major schemas needing improvement across every page type. Through the schema automation D2C is pursuing, we are building a structure that generates and applies the major schemas consistently rather than handling them country by country and page by page.',
  d4Title: 'Citable Content needs both a wider range of AI-answer-ready formats and better author attribution',
  d4: 'Content scored 29.1, the lowest of all areas. There is not enough of the content AI can readily use as a direct answer \u2014 FAQ Blocks, Summary Boxes, Definition Paragraphs \u2014 and FAQ Blocks in particular are being driven as a key improvement task across the business units and Customer Value Innovation.\nCitable Sentences (sentences containing numbers, years, statistics, or research keywords) were met at 53.6% overall. Germany, Spain, Australia, and Canada exceeded 80%, while the remaining countries were relatively low \u2014 a wide country-level spread. Comparing German sites in July, LG.com met Citable Sentence at 86.1% versus Samsung.com\u2019s 19.4%, confirming it as a competitive advantage for LG content. Expanding Citable Sentences in the lower-scoring countries \u2014 India, Mexico, Vietnam, and the USA \u2014 would reinforce that advantage.\nAuthor / source / date attribution was also low at 34.9%. Because AI uses these signals to judge credibility and recency, informational content in particular should be reinforced. Press & Media, where these signals matter most, was comparatively sound at 74.3%, while Buying Guide and LG Experience sat at 0% and need urgent attention.',
}
