...
function insightV3Parts(meta = {}, lang = 'ko', productsCnty = [], assetBase = '') {
  ...
  const ex3Ko = `'26년 7월부터 Readability 평가 체계와 대시보드를 새롭게 구축해 LG.com 콘텐츠가 AI가 읽고 활용하기 좋은 상태인지 함께 점검하고자 합니다.<br/><br/>전략 10개 국가 및 글로벌 대표 사이트의 9개 주요 페이지 유형, 총 9,284개 페이지를 대상으로 AI 봇의 1) 사이트 성능, 2) 웹접근성, 3) Basic SEO, 4) 스키마마크업, 5) 고인용 콘텐츠, 6) AI Crawlability 총 6개 영역에서 첫 평가를 진행한 결과, <strong style="color:#FFFFFF;">LG.com의 전체 Readability 평균은 79.7점</strong>으로 나타났습니다.<br/><br/>영역별로는 사이트 성능 99.4점, AI Crawlability 95.4, Basic SEO 89.2점, 웹접근성 83.5점으로 기본적인 페이지 접근성과 기술 환경은 비교적 양호했습니다. 반면, <strong style="color:#FFFFFF;">스키마마크업 33.2점, 고인용 콘텐츠 56.3점</strong>은 상대적으로 낮아 개선이 필요한 영역으로 확인됐습니다.<br/><br/><span style="color:#FDA4AF;">이에 대응하여 FAQ 등 AI 답변에 적합한 콘텐츠 형식 확대, 스키마마크업 자동화, Support 및 PDP 주요 정보의 초기 제공 구조 개선(SSR)을 지속 수행하고자 합니다. 빠른 개선이 필요한 영역은 9월 Committee에 bring-up하고, Visibility 상승으로 이어질 수 있는 기반을 강화할 계획입니다.</span>`
...
function actionItemsV2SectionHtml(meta = {}, lang = 'ko', categoryStats = null) {
  ...
  // ── 9월 진행 사항 — 조직별 정리 (선·색 없는 표, 조직명 강조 / 사용자 제공 텍스트 그대로, 2026-09-21) ──
  ...
  const nRow = (org, body) => `<tr><td width="90" style="${nOrg}">${org}</td><td style="${nTxt}">${body}</td></tr>`
  const nextKo = `<table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed;border-collapse:collapse;">
    ${nRow('HS', `• REF : Why LG(Ice Solution) 마이크로사이트 업로드 (VN)<br/>• Cooking : MWO PLP FAQ 추가 (10개국 완료)<br/>• 공통 : 냉장고, 세탁기, 식기세척기, 쿠킹 FAQ 컨텐츠 모음 페이지 오픈 (IN)<br/>• IFA 2026 글로벌 보도자료 제작 및 배포 5건, 미디어 취재 지원을 통한 글로벌 커버리지 4,516건 확보<br/>• GEO 관점의 Corporate Narrative Engine Kit 내 AI Home 내러티브 검토 및 콘텐츠 개발 지원<br/>• Youtube 바이럴 시딩 캠페인 GEO 대응 업로드 (냉장고)`)}
    ${nRow('ES', `• RAC - 구매 여정 기반 정보성 인용 영역 확대 위한 LG.com 바잉 가이드 제작 완료 (8월 배포)<br/>• Air Care - 구매 여정 기반 정보성 인용 영역 확대 위한 LG.com 바잉 가이드 및 아티클 4종 등 총 5종 제작 완료 (배포 완료)`)}
    ${nRow('MS', `• TV - 레딧 Micro RGB 메가쓰레드 IFA PR 확산 통한 AI TV/Picture Quality/ Ultra Large 소구, Reflection Free/ Dolby Vision 체험단 후기 확산 (완료), OLED–Black Level, QNED–Ultra Large 메가쓰레드 및 C6H 83" Brightness RRM 리뷰 (예정) / 닷컴 OCP(OLED Care Program) 관련 FAQ 업데이트 (진행 중)<br/>• 모니터 - 고주사율 (240Hz, 1000H) 메가쓰레드 발행 (완료), 체험단 리뷰 발행(GM9, GX9) (진행중), 고화질(5K2K) 메가쓰레드 발행 및 25G590B 체험단 모집/IRM리뷰 확산 (예정) / UltraGear evo 및 Gaming Monitor FAQ 글로벌 적용 (완료)`)}
    ${nRow('D2C', `• 신규 Chat GPT 및 AI MAX 신규 광고 상품에 대한 PoC 진행 중 (상품의 마케팅 효과성 및 운영 목적에 대한 각 본부별 협업으로 진행 중)<br/>• AI 기반의 LLM 모델 최적화 자동 진단 및 콘텐츠 수정 Agent의 Global PoC 진행 중<br/>&nbsp;&nbsp;: 영국 대상 Gemini / ChatGPT 답변 반영 확인 및 성과 모니터링 진행 완료(Visibility 개선 확인), 추가 국가로 전략국가 9개국 확산 진행 중(미국제외)<br/>• 스키마 마크업 자동화 통합가이드 제작 완료 및 최종 개발 요건 검토(총 9종)<br/>• 제품 Review 영역의 Server-Side-Rendering 글로벌 Roll-Out (AI가 읽을 수 있는 형태로 Review 데이터 변경)`)}
    ${nRow('글로벌<br/>커뮤니케이션팀', `• 전사 핵심 사업 과제 별 기업 내러티브 구성 및 컨텐츠/활동 기획:<br/>&nbsp;&nbsp;- GEO 향상 위한 Corporate Narrative Engine Kit 제작 및 공유 세션 기획 (진행 중)<br/>&nbsp;&nbsp;- 글로벌사우스 : 브라질 공장 개소 연계 CEO 칼럼 발행 (게재 완료)<br/>&nbsp;&nbsp;- AI DC 냉각솔루션 : IDC 리서치 프로젝트 기획, DCW Asia 연계 CEO 칼럼 및 행사 연계 미디어 활동 기획 (w/ ES본부) (진행 중)<br/>• IFA 2026 연계 보도자료 제작 및 법인 미디어 활동 기획 지원 (진행 중)<br/>• 미국 뉴스룸 플랫폼 재정비 지원 (9/1 오픈 기준으로 개발 진행 중)`)}
    ${nRow('고객가치혁신', `• LG.com Support 페이지 SSR(Server-Side Rendering) 전환 진행 중<br/>&nbsp;&nbsp;: 7/20 닷컴 SSR 전환 과제 Kick Off 미팅 완료<br/>&nbsp;&nbsp;: 8월 법인별 Legacy 콘텐츠에 적용된 비표준 기능 및 소스 정의 후 클렌징 작업 진행 중<br/>&nbsp;&nbsp;: 본사 표준 콘텐츠 가이드 제작 및 전사 표준 문서 개정본 제작 중 (8/27 법인 배포 완료)`)}
  </table>`
  const nextEn = `<table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed;border-collapse:collapse;">
    ${nRow('HS', `• REF: Why LG (Ice Solution) microsite uploaded (VN)<br/>• Cooking: MWO PLP FAQs added (10 countries done)<br/>• Common: FAQ hub page opened for refrigerator, washer, dishwasher & cooking (IN)<br/>• IFA 2026: 5 global press releases produced & distributed; 4,516 global coverage items secured via media relations support<br/>• Reviewed the AI Home narrative in the Corporate Narrative Engine Kit from a GEO perspective & supported content development<br/>• YouTube viral seeding campaign uploaded with GEO alignment (refrigerator)`)}
    ${nRow('ES', `• RAC - LG.com buying guide completed to expand informative citation areas along the purchase journey (deployed in Aug)<br/>• Air Care - 5 items completed incl. LG.com buying guide & 4 articles to expand informative citation areas (deployment done)`)}
    ${nRow('MS', `• TV - Reddit Micro RGB megathread with IFA PR amplification for AI TV/Picture Quality/Ultra Large; Reflection Free/Dolby Vision tester reviews amplified (done); OLED–Black Level & QNED–Ultra Large megathreads and C6H 83" Brightness RRM reviews (planned) / dotcom OCP (OLED Care Program) FAQ update (in progress)<br/>• Monitor - high-refresh (240Hz, 1000H) megathread published (done), tester reviews published (GM9, GX9) (in progress), high-res (5K2K) megathread & 25G590B tester recruiting/IRM review amplification (planned) / UltraGear evo & Gaming Monitor FAQs applied globally (done)`)}
    ${nRow('D2C', `• PoC for new ChatGPT & AI MAX ad products in progress (cross-BU collaboration on marketing effectiveness & operating purpose)<br/>• Global PoC of the AI-based LLM-optimization auto-diagnosis & content-fix Agent in progress<br/>&nbsp;&nbsp;: UK Gemini/ChatGPT answer reflection & performance monitoring done (Visibility improvement confirmed); expansion to 9 strategic countries in progress (excl. US)<br/>• Integrated schema-markup automation guide completed & final dev-requirement review (9 types total)<br/>• Global roll-out of Server-Side-Rendering for the product Review area (making review data AI-readable)`)}
    ${nRow('Global Comm<br/>Team', `• Building corporate narratives and planning content/activities for key company-wide business tasks:<br/>&nbsp;&nbsp;- Corporate Narrative Engine Kit for GEO gains & sharing session planning (in progress)<br/>&nbsp;&nbsp;- Global South: CEO column tied to the Brazil plant opening (published)<br/>&nbsp;&nbsp;- AI DC cooling solutions: IDC research project planning, DCW Asia-linked CEO column & event-linked media activities (w/ ES division) (in progress)<br/>• IFA 2026 press releases & subsidiary media activity planning support (in progress)<br/>• US newsroom platform re-organization support (development in progress for the 9/1 open)`)}
    ${nRow('CVI', `• LG.com Support page SSR (Server-Side Rendering) conversion in progress<br/>&nbsp;&nbsp;: 7/20 dotcom SSR conversion kick-off meeting done<br/>&nbsp;&nbsp;: Aug - defining non-standard features & sources in per-subsidiary legacy content, then cleansing in progress<br/>&nbsp;&nbsp;: HQ standard content guide & company-wide standard document revision in production (distributed to subsidiaries 8/27)`)}
  </table>`
  const nextBlock = `<table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed;border:1px solid #E8EDF2;border-radius:10px;margin-bottom:8px;">
      ...
      <tr><td style="padding:12px 14px;word-break:break-word;">
        ${/* 필드명 3→4 (2026-09-21): 9월 표 구조 교체(HS 신설·고객가치혁신 분리) — 옛 편집본(todoV2NextHtml3)이 새 기본값을 덮지 않게 리셋 */''}
        ${edWrapT('todoV2NextHtml4', L(nextKo, nextEn))}
      </td></tr>
    </table>`
