        ...
        <td style="padding:5px 6px 3px;">
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
              ...
              <td align="right" style="vertical-align:middle;">
                <table border="0" cellpadding="0" cellspacing="0" align="right" style="float:right;"><tr>
                  <td style="vertical-align:middle;white-space:nowrap;"><span style="font-size:13px;font-weight:700;color:${ratioColor};font-family:${EM_FONT};letter-spacing:-1px;">${escapeHtml(compShort(mainCompName) || 'SS')} ${curRatio}%</span></td>
...
// ─── 반기 레터 (Semi-Annual Letter) — 별도 레이아웃 ──────────────────────────
// 섹션: 1) 반기 하이라이트  2) 반기 누적 성과  3) 카테고리별 반기 추이
...
export function generateSemiAnnualEmailHTML(meta, total, products, citations, dotcom = {}, lang = 'ko', productsCnty = [], citationsCnty = [], options = {}) {
  ...
  const t = T[lang] || T.ko
  total = total || { score: 0, prev: 0, vsComp: 0 }
  products = products || []
  productsCnty = productsCnty || []

  const L = {
    ko: { hl: '반기 하이라이트', cum: '반기 누적 성과', trend: '카테고리별 반기 추이', rising: '상승 주도 카테고리', falling: '하락 주도 카테고리', bu: '사업본부', country: '국가', vs: '대비', period6: '최근 6개월 추이', noTrend: '추이 데이터 없음', overall: '전체 GEO Visibility', gap: '삼성 대비 격차', reportType: 'GEO 반기 성과 분석 리포트', title: '생성형 AI 엔진 가시성(Visibility) 반기 성과 분석', lgh: 'LG', ss: 'Samsung' },
    en: { hl: 'Semi-Annual Highlights', cum: 'Semi-Annual Cumulative Performance', trend: 'Category Semi-Annual Trend', rising: 'Top Risers', falling: 'Top Decliners', bu: 'Division', country: 'Country', vs: 'vs', period6: 'Last 6 months', noTrend: 'No trend data', overall: 'Total GEO Visibility', gap: 'Gap vs Samsung', reportType: 'GEO Semi-Annual Performance Report', title: 'Generative AI Engine Visibility Semi-Annual Analysis', lgh: 'LG', ss: 'Samsung' },
  }
  const L2 = L[lang] || L.ko

  const prodName = (p) => {
    const id = (p.id || '').toLowerCase()
    return lang === 'en' ? (PROD_ID_TO_EN[id] || p.kr || p.label || p.id) : (PROD_ID_TO_KR[id] || p.kr || p.label || p.id)
  }
  const sortedProducts = [...products].sort((a, b) =>
    (PROD_ID_TO_ORDER[(a.id || '').toLowerCase()] ?? 99) - (PROD_ID_TO_ORDER[(b.id || '').toLowerCase()] ?? 99))

  // 카테고리별 6개월 시계열 — productsCnty.monthlyScores 를 product 로 묶어 date 별 평균 (data.md §5.7)
  function halfSeries(p) {
    const pid = (p.id || '').toLowerCase()
    const rows = productsCnty.filter(r => {
      const raw = String(r.product || '').trim()
      const id = NAME_TO_PROD_ID[raw] || raw.toLowerCase()
      return id === pid
    })
    const byDate = {}
    rows.forEach(r => (r.monthlyScores || []).forEach(m => {
      if (m.score == null) return
      if (!byDate[m.date]) byDate[m.date] = { sum: 0, n: 0 }
      byDate[m.date].sum += Number(m.score) || 0
      byDate[m.date].n++
    }))
    const canonical = (rows[0]?.monthlyScores || []).map(m => m.date)
    const dates = Object.keys(byDate).sort((a, b) => canonical.indexOf(a) - canonical.indexOf(b))
    let series = dates.map(d => +(byDate[d].sum / byDate[d].n).toFixed(1))
    if (!series.length) series = (p.monthly && p.monthly.length) ? p.monthly.slice() : (p.prev != null && p.score != null ? [p.prev, p.score] : (p.score != null ? [p.score] : []))
    return series.slice(-6)
  }

  const seriesList = sortedProducts.map(p => ({ p, s: halfSeries(p) }))
  const flat = seriesList.flatMap(x => x.s)
  const gMax = flat.length ? Math.max(...flat) : 100
  const gMin = flat.length ? Math.min(...flat) : 0

  // 전체 수치
  const compAvg = total.vsComp || 0
  const gap = +((total.score || 0) - compAvg).toFixed(1)
  const totalRatio = compAvg > 0 ? Math.round((total.score || 0) / compAvg * 100) : 100
  const totalSignal = totalRatio >= 100 ? '#22C55E' : totalRatio >= 80 ? '#F59E0B' : '#EF4444'
  const scoreBarW = Math.round(total.score || 0)

  // 상승/하락 주도 카테고리
  const movers = sortedProducts
    .filter(p => p.prev != null && p.score != null && p.prev !== 0)
    .map(p => ({ name: prodName(p), d: +(p.score - p.prev).toFixed(1) }))
  const risers = movers.filter(m => m.d > 0).sort((a, b) => b.d - a.d).slice(0, 3)
  const fallers = movers.filter(m => m.d < 0).sort((a, b) => a.d - b.d).slice(0, 3)
  const moverRows = (arr, color) => arr.length
    ? arr.map(m => `<tr><td style="padding:4px 0;font-size:13px;color:#1A1A1A;font-family:${EM_FONT};">${escapeHtml(m.name)}</td><td align="right" style="padding:4px 0;font-size:13px;font-weight:700;color:${color};font-family:${EM_FONT};">${m.d > 0 ? '+' : ''}${m.d.toFixed(1)}%p</td></tr>`).join('')
    : `<tr><td style="padding:4px 0;font-size:12px;color:#94A3B8;font-family:${EM_FONT};">—</td></tr>`

  // 누적 성과 테이블 행 (본부/국가 공통)
  const cumHeader = `<tr style="background:#F8FAFC;">
    <th align="left" style="padding:8px 10px;border-bottom:2px solid ${EM_RED};font-size:12px;color:#475569;font-family:${EM_FONT};"> </th>
    <th align="right" style="padding:8px 10px;border-bottom:2px solid ${EM_RED};font-size:12px;color:#475569;font-family:${EM_FONT};">${L2.lgh}</th>
    <th align="right" style="padding:8px 10px;border-bottom:2px solid ${EM_RED};font-size:12px;color:#475569;font-family:${EM_FONT};">${L2.ss}</th>
    <th align="right" style="padding:8px 10px;border-bottom:2px solid ${EM_RED};font-size:12px;color:#475569;font-family:${EM_FONT};">${L2.vs}</th>
    <th align="right" style="padding:8px 10px;border-bottom:2px solid ${EM_RED};font-size:12px;color:#475569;font-family:${EM_FONT};">MoM</th>
  </tr>`
  const cumRow = (name, cur, prev) => {
    const lg = cur?.lg, comp = cur?.comp
    const ratio = comp > 0 ? Math.round(lg / comp * 100) : 100
    const rColor = ratio >= 100 ? '#16A34A' : ratio >= 80 ? '#E8910C' : '#DC2626'
    const d = (prev && prev.lg != null && lg != null) ? +(lg - prev.lg).toFixed(1) : null
    return `<tr>
      <td style="padding:8px 10px;border-bottom:1px solid #F1F5F9;font-size:14px;color:#1A1A1A;font-family:${EM_FONT};font-weight:600;">${escapeHtml(name)}</td>
      <td align="right" style="padding:8px 10px;border-bottom:1px solid #F1F5F9;font-size:14px;color:#1A1A1A;font-family:${EM_FONT};font-weight:700;">${lg != null ? lg + '%' : '—'}</td>
      <td align="right" style="padding:8px 10px;border-bottom:1px solid #F1F5F9;font-size:14px;color:#3B82F6;font-family:${EM_FONT};">${comp != null ? comp + '%' : '—'}</td>
      <td align="right" style="padding:8px 10px;border-bottom:1px solid #F1F5F9;font-size:14px;color:${rColor};font-family:${EM_FONT};font-weight:700;">${L2.vs} ${ratio}%</td>
      <td align="right" style="padding:8px 10px;border-bottom:1px solid #F1F5F9;font-family:${EM_FONT};">${d == null ? '<span style="color:#94A3B8;font-size:13px;">—</span>' : deltaHtml(d, 13)}</td>
    </tr>`
  }
  const buTotals = total.buTotals || {}, buTotalsPrev = total.buTotalsPrev || {}
  const buRows = ['MS', 'HS', 'ES'].filter(k => buTotals[k]).map(k => cumRow(lang === 'en' ? k : k + '본부', buTotals[k], buTotalsPrev[k])).join('')
  const countryTotals = total.countryTotals || {}, countryTotalsPrev = total.countryTotalsPrev || {}
  const ctRows = Object.keys(countryTotals).map(c => cumRow(cntyLabel(c, lang), countryTotals[c], countryTotalsPrev[c])).join('')

  // 카테고리 카드 (2열)
  const catCard = ({ p, s }) => {
    const st = statusInfo(p.status, lang)
    const ratio = p.compRatio || (p.vsComp > 0 ? Math.round(p.score / p.vsComp * 100) : 100)
    const bar = s.length ? monthlyTrendHtml(s, EM_RED, gMax, gMin) : `<span style="font-size:12px;color:#94A3B8;font-family:${EM_FONT};">${L2.noTrend}</span>`
    return `<table border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#FFFFFF;border:2px solid ${st.border};border-radius:12px;">
      <tr><td style="padding:14px 16px;">
        <table border="0" cellpadding="0" cellspacing="0" width="100%"><tr>
          <td style="font-size:15px;font-weight:700;color:#1A1A1A;font-family:${EM_FONT};">${escapeHtml(prodName(p))}</td>
          <td align="right"><span style="background:${st.bg};color:${st.color};border:1px solid ${st.border};border-radius:6px;padding:2px 8px;font-size:11px;font-weight:700;font-family:${EM_FONT};">${st.label}</span></td>
        </tr></table>
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:6px;"><tr>
          <td><span style="font-size:24px;font-weight:900;color:#1A1A1A;font-family:${EM_FONT};">${p.score != null ? p.score : '—'}</span><span style="font-size:12px;color:#94A3B8;font-family:${EM_FONT};"> %</span>
          &nbsp;<span style="font-size:12px;font-weight:700;color:${ratio >= 100 ? '#16A34A' : ratio >= 80 ? '#E8910C' : '#DC2626'};font-family:${EM_FONT};">${L2.vs} ${ratio}%</span></td>
        </tr></table>
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:10px;"><tr><td align="center">${bar}</td></tr></table>
        <table border="0" cellpadding="0" cellspacing="0" width="100%"><tr><td style="font-size:10px;color:#94A3B8;font-family:${EM_FONT};padding-top:4px;">${L2.period6}</td></tr></table>
      </td></tr>
    </table>`
  }
  const cards = seriesList.map(catCard)
  let catRows = ''
  for (let i = 0; i < cards.length; i += 2) {
    catRows += `<tr>
      <td width="50%" style="vertical-align:top;padding:6px;">${cards[i]}</td>
      <td width="50%" style="vertical-align:top;padding:6px;">${cards[i + 1] || ''}</td>
    </tr>`
  }

  const reportTypeLabel = meta.reportType || L2.reportType
  const titleLabel = meta.title || L2.title
  const dateLineLabel = meta.dateLine || (lang === 'en' ? 'As of ' + (meta.period || '') : (meta.period || '') + ' 기준')
  const sectionBar = (label) => `<table border="0" cellpadding="0" cellspacing="0"><tr>
    <td width="3" style="background:${EM_RED};border-radius:2px;">&nbsp;</td>
    <td style="padding-left:8px;font-size:19px;font-weight:700;color:#1A1A1A;font-family:${EM_FONT};">${label}</td>
  </tr></table>`
...
<head>
  ...
  <title>LG GEO Semi-Annual ${escapeHtml(meta.period)}</title>
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="${containerWidth}" style="width:${containerWidth}px;max-width:${containerWidth}px;table-layout:fixed;background:#FFFFFF;border-radius:16px;font-family:${EM_FONT};">
        ...
        <tr><td style="padding:8px 16px 28px;">
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            <!-- ══ 1. 반기 하이라이트 ══ -->
            <tr><td style="padding-bottom:14px;">${sectionBar(L2.hl)}</td></tr>
            <tr><td style="padding-bottom:24px;">
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#0F172A;border-radius:14px;">
                <tr><td style="padding:22px 24px;border-top:4px solid ${totalSignal};border-radius:14px;">
                  <table border="0" cellpadding="0" cellspacing="0" width="100%"><tr>
                    <td style="vertical-align:bottom;">
                      <span style="font-size:13px;color:#94A3B8;font-family:${EM_FONT};">${L2.overall}</span><br/>
                      <span style="font-size:48px;font-weight:900;color:#FFFFFF;font-family:${EM_FONT};">${total.score != null ? total.score : '—'}</span>
                      <span style="font-size:20px;color:#94A3B8;font-family:${EM_FONT};"> %</span>
                    </td>
                    <td align="right" style="vertical-align:bottom;">
                      ${compAvg > 0 ? `<span style="font-size:16px;color:#3B82F6;font-weight:800;font-family:${EM_FONT};">${L2.ss} ${compAvg}%</span><br/>
                      <span style="font-size:16px;font-weight:800;color:${totalSignal};font-family:${EM_FONT};">${L2.gap} ${gap > 0 ? '+' : ''}${gap}%p (${L2.vs} ${totalRatio}%)</span>` : ''}
                    </td>
                  </tr></table>
            ...
            <tr><td style="padding-bottom:28px;">
              <table border="0" cellpadding="0" cellspacing="0" width="100%"><tr>
                <td width="50%" style="vertical-align:top;padding-right:8px;">
                  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;"><tr><td style="padding:14px 16px;">
                    <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#15803D;font-family:${EM_FONT};">▲ ${L2.rising}</p>
                    <table border="0" cellpadding="0" cellspacing="0" width="100%">${moverRows(risers, '#16A34A')}</table>
                  </td></tr></table>
                </td>
                <td width="50%" style="vertical-align:top;padding-left:8px;">
                  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#FFF1F2;border:1px solid #FECDD3;border-radius:12px;"><tr><td style="padding:14px 16px;">
                    <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#BE123C;font-family:${EM_FONT};">▼ ${L2.falling}</p>
            ...
            <!-- ══ 2. 반기 누적 성과 ══ -->
            <tr><td style="padding-bottom:14px;">${sectionBar(L2.cum)}</td></tr>
            ${buRows ? `<tr><td style="padding-bottom:20px;">
              <p style="margin:0 0 6px;font-size:14px;font-weight:700;color:#475569;font-family:${EM_FONT};">${L2.bu}</p>
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #E8EDF2;border-radius:10px;">${cumHeader}${buRows}</table>
            </td></tr>` : ''}
            ${ctRows ? `<tr><td style="padding-bottom:28px;">
              <p style="margin:0 0 6px;font-size:14px;font-weight:700;color:#475569;font-family:${EM_FONT};">${L2.country}</p>
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #E8EDF2;border-radius:10px;">${cumHeader}${ctRows}</table>
            </td></tr>` : ''}

            <!-- ══ 3. 카테고리별 반기 추이 ══ -->
            <tr><td style="padding-bottom:14px;">${sectionBar(L2.trend)}</td></tr>
            <tr><td>
              <table border="0" cellpadding="0" cellspacing="0" width="100%">${catRows}</table>
            </td></tr>

            <tr><td style="padding-top:16px;">${dashboardLinkButtonHtml(lang)}</td></tr>
          </table>
        </td></tr>
