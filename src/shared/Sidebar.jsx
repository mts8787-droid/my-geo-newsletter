...
import { LG_RED, FONT, META_TEXT_KEYS } from './constants.js'
...
// EN 번역 필드 목록·병합은 순수 모듈로 분리 (서버 게시 엔진 공용) — enMeta.js
import { isEnTextField, mergeEnMeta } from './enMeta.js'
export { isEnTextField, mergeEnMeta }
  ...
  async function handleGsSync() {
    ...
    try {
      ...
      if (parsed.meta) {
        ...
        const textKeys = META_TEXT_KEYS  // single source — constants.js (서버 게시 엔진과 공유)
        setMetaKo(m => {
          const merged = { ...m }
      ...
      if (onSyncExtra) {
        const syncExtra = {
          ...
          citDomainByLlmTrend: parsed.citDomainByLlmTrend || null,
  ...
  return (
    <div style={{ width: 520, minWidth: 520, borderRight: '1px solid #1E293B',
      ...
      <div style={{ padding: '16px 14px', flex: 1, overflowY: 'auto' }}>
        ...
        {/* 웹 게시 (KO+EN 동시) — 통합 게시로 일원화된 모드(visibility 등)는 숨김 */}
        {hidePublish ? (
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
          <div style={{ flex: 1 }}>
            <p style={{ margin: '0 0 3px', fontSize: 11, color: '#64748B', fontFamily: FONT }}>보고서 번호 <span style={{ color: '#334155' }}>(자동)</span></p>
