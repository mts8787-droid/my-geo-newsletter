...
export const CHECKLIST_EN = {
  ...
  '10': { name: 'Semantic HTML', def: 'Use of meaning-bearing HTML elements that express content role and structure', pass: 'Headings + landmarks total 8 or more (main not required)' , method: 'main (role=main accepted), nav, header, footer, article, section, aside plus heading tags' },
  '11': { name: 'Heading Hierarchy', def: 'Whether heading tags form a proper hierarchy separating titles from sub-content', pass: 'No inverted headings in the body' , method: 'Fails only when a level above the first body heading appears later (GNB/footer headings excluded, level jumps allowed)' },
  ...
  '15': { name: 'Canonical', def: 'URL setting that tells search engines which page is authoritative', pass: 'Present + same host (path differences allowed)' , method: 'Verify link[rel=\"canonical\"] host matches' },
  ...
  '23': { name: 'Schema Types — FAQ', def: 'Frequently asked questions and answers marked up so AI can read them', pass: 'All required JSON-LD fields present, parses cleanly' , method: 'mainEntity present — only pages with an FAQ section are assessed (otherwise N/A)' },
  ...
  '26': { name: 'Schema Types — ImageObject', def: 'Image URL, name, and caption marked up so AI can read them', pass: 'All required JSON-LD fields present, parses cleanly' , method: 'url, name, description, uploadDate present — only pages with body content images are assessed (otherwise N/A)' },
  '27': { name: 'Schema Types — VideoObject', def: 'Video URL, title, and thumbnail marked up so AI can read them', pass: 'All required JSON-LD fields present, parses cleanly' , method: 'url, name, description, thumbnailUrl present — only pages with a video are assessed (otherwise N/A)' },
  '28': { name: 'Schema Types — HowTo', def: 'Instructions made of materials and steps marked up so AI can read them', pass: 'All required JSON-LD fields present, parses cleanly' , method: 'HowToSupply / HowToStep present — only troubleshooting pages with step lists are assessed (otherwise N/A)' },
  '29': { name: 'Schema Types — Article', def: 'Article title, author, publisher, and body marked up so AI can read them', pass: 'All required JSON-LD fields present, parses cleanly' , method: 'headline, author, publisher, articleBody present — only pages with an article structure are assessed (otherwise N/A)' },
  ...
  '34': { name: 'Author / Source', def: 'Author, or source and date, showing who wrote the content and when', pass: 'Author, or (source + date), present' , method: 'JSON-LD author, or (datePublished + publisher) — assessed only on newsroom and press/media page types' },
  '35': { name: 'Summary Box*', def: 'Whether a short summary of the key points sits near the top of the body', pass: 'One or more present' , method: 'Heading/label phrases (At a Glance, Key features, summary, etc., multilingual) or summary block selectors (p.info-desc, AEM PDP c-floating-features) — class-name matching removed as a false-positive source' },
  '36': { name: 'Citable Sentences*', def: 'Whether enough sentences carry numbers, years, or statistics that AI can cite as evidence', pass: '5 or more' , method: 'Sentences with numbers, years, statistics, certifications, or verification/authority phrasing (EEAT: testing, patents, expert endorsement, clinical proof)' },
  ...
  '40': { name: 'Image File Name', def: 'Whether the image file name alone identifies the brand and product', pass: '30% or more images with brand/product keywords' , method: 'Verify image file name keywords (logo/icon files excluded)' },
  ...
  '42': { name: 'Status Code (Soft 404)', def: 'Whether a page that responds normally is actually empty', pass: 'No 404-style wording in the body of a 200 response' , method: 'Detect 404 phrases in the body (body under 200 chars is a secondary signal)' },
