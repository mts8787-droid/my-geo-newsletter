...
const STYLE_TYPES = new Set(['monthlyReportBody', 'monthlyDelta', 'totalInsight', 'product', 'cnty', 'modelDelta', 'highlight', 'bump'])

// Citation 외부채널·도메인 범프 — 월별 값(순위/카운트) 요약
function bumpLines(data) {
  const months = data.citDomainMonths || data.citTrendMonths || []
  const fmt = (obj, label) => {
    const names = Object.keys(obj || {})
    if (!names.length) return ''
    const lines = names.slice(0, 10).map(n => { const m = obj[n] || {}; return `  - ${n}: ${months.map(mo => `${mo} ${m[mo] != null ? m[mo] : '-'}`).join(' / ')}` })
    return `[${label}]\n${lines.join('\n')}`
  }
  return [fmt(data.citTouchPointsTrend, '외부채널 범프(카테고리)'), fmt(data.citDomainTrend, '도메인 범프')].filter(Boolean).join('\n\n') || '(범프 데이터 없음)'
}

// Highlight — 주간 경쟁사 트렌드 (TV/냉장고/세탁기/에어컨) LG vs 상위3 경쟁사 요약
const HL_PRODS_P = ['tv', 'fridge', 'washer', 'rac']
const HL_KR = { tv: 'TV', fridge: '냉장고', washer: '세탁기', rac: '에어컨' }
function highlightLines(data) {
  const wa = data.weeklyAll || {}
  const latestOf = arr => { const v = (arr || []).filter(x => x != null); return v.length ? v[v.length - 1] : null }
  const prevOf = arr => { const v = (arr || []).filter(x => x != null); return v.length >= 2 ? v[v.length - 2] : null }
  const out = []
  HL_PRODS_P.forEach(pid => {
    const src = wa[pid] || {}; const w = src['Total'] || src['TTL'] || src['TOTAL']; if (!w || !Array.isArray(w.LG)) return
    const comps = Object.keys(w).filter(b => b !== 'LG').map(b => ({ b, v: latestOf(w[b]) })).filter(x => x.v != null).sort((a, b) => b.v - a.v).slice(0, 3)
    const lgL = latestOf(w.LG), lgP = prevOf(w.LG)
    const lgWow = (lgL != null && lgP != null) ? ` (WoW ${(lgL - lgP) > 0 ? '+' : ''}${(lgL - lgP).toFixed(1)})` : ''
    const compStr = comps.map(c => `${c.b} ${c.v != null ? c.v.toFixed(1) : '—'}`).join(' / ')
    out.push(`[공식수치] ${HL_KR[pid] || pid}: LG ${lgL != null ? lgL.toFixed(1) : '—'}${lgWow} | 경쟁사 Top3: ${compStr || '없음'}`)
  })
...
${NO_UNLAUNCHED_COMMENT}`
  ...
  if (type === 'bump') {
    return `[섹션: Citation 외부채널·도메인 범프차트 — 순위/추세 변화 분석]
아래 [공식수치](월별 값)만 사용해 외부채널(카테고리)과 도메인의 순위·추세 변화를 분석하세요. 상승·하락 항목을 짚고 시사점을 정리. 절대 새로 계산·추정 금지.

${bumpLines(data)}

[주의: 위 리스트에 없는 항목·수치 언급 금지.]`
  }

  if (type === 'highlight') {
    const lines = highlightLines(data)
    return `[섹션: Highlight Insight — 주요 4개 제품(TV·냉장고·세탁기·에어컨) 주간 경쟁사 트렌드 분석]
아래 [공식수치]만 사용해 주요 4개 제품의 주간 Visibility에서 LG와 상위 3개 경쟁사의 최신 현황·추세를 분석하세요. 절대 새로 계산·추정하지 마세요.

[필수]
- 제품별로 LG가 경쟁사 대비 선도/추격/열위 중 어디인지 (최신 주간 값 비교)
- LG의 주간 흐름(WoW) 특이사항
- 경쟁사 중 위협적인 곳 짚기

${lines}

[주의: 위 [공식수치] 외 임의 계산·추정 금지. 리스트에 없는 항목 언급 금지.]`
  }
