...
export async function generateAIInsight(type, data, lang = 'ko', rules = '', extraPrompt = '') {
  try {
    const r = await fetch('/api/generate-insight', {
      method: 'POST', headers: JSON_HEADERS,
      body: JSON.stringify({ type, data, lang, rules, extraPrompt }),
    })
...
export async function publishCombinedDashboard(generateDashboardHTML, resolveDataForLang, options = {}) {
  const { includeReadability = false } = options
  ...
  const meta = d.meta || {}
  const total = d.total || {}
  // meta.period는 하위 대시보드(Visibility Editor)에서 설정한 값을 그대로 사용
  ...
  const opts = { monthlyVis: d.monthlyVis || [], includeReadability }
