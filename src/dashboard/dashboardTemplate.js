...
function prVisibilityTabHtml(weeklyPR, weeklyPRLabels, lang, meta, extra, period = 'weekly') {
  ...
  (function(){
    ...
    function renderMatrix(){
      ...
      h+='<th style="padding:8px 10px;text-align:center;font-size:16px;font-weight:700;color:#64748B;border-bottom:2px solid #E8EDF2;min-width:80px">${lang==='en'?'Competitor':'경쟁사'}</th>';
    ...
    function renderSections(){
      ...
      sectionTopics.forEach(function(st0){
        ...
        if(fView==='together'&&t2){
          html+='<div style="font-size:14px;font-weight:700;color:#64748B;margin:24px 0 2px">${lang==='en'?'By Country (vs #1 ratio)':'국가별 (1위 경쟁사 경쟁비)'}</div>';
          html+=t2;
        }
        html+='</div></div>';
