  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #E2E8F0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 4, height: 22, borderRadius: 4, background: '#CF0652', flexShrink: 0 }} /><h3 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: 0 }}>{t(lang, 'rawGoalTitle')}</h3></div>
      </div>
          ...
          <tbody>
            ...
            {filtered.length > 0 && (
              <tr style={{ background: '#F8FAFC', borderTop: '2px solid #CBD5E1' }}>
                <td style={{ padding: '9px 12px', position: 'sticky', left: 0, zIndex: 10, background: '#F8FAFC', fontWeight: 700, color: '#111827' }} colSpan={4}>{t(lang, 'total')}</td>
                {MONTHS.map(m => {
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={4 + MONTHS.length + 1} style={{ padding: '32px 12px', textAlign: 'center', color: '#94A3B8', fontSize: 16 }}>
                  {t(lang, 'noDataForSH')}
                </td>
              </tr>
            )}
