...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', fontSize: 13, fontFamily: FONT }}>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#111827', marginBottom: 2 }}>월별 목표 대비 실적</h3>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <ComposedChart data={monthlyTotals} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
            ...
            <XAxis dataKey="month" tick={{ fill: '#475569', fontSize: 12, fontFamily: FONT }} tickLine={false} axisLine={{ stroke: '#E2E8F0' }} />
            <YAxis tick={{ fill: '#94A3B8', fontSize: 11, fontFamily: FONT }} tickLine={false} axisLine={false} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v} />
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ fontSize: 12, fontFamily: FONT }} iconType="circle" iconSize={7} />
            <Bar dataKey="actual" name="실적" fill={RED} radius={[4, 4, 0, 0]} maxBarSize={32} />
            <Line dataKey="goal" name="목표" type="monotone" stroke={BLUE} strokeWidth={2} dot={{ r: 3.5, fill: BLUE, stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 5 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#111827', marginBottom: 2 }}>연간 누적 목표대비 실적</h3>
            <p style={{ fontSize: 12, color: '#64748B' }}>~{selectedMonth} 실적 · 12월까지 목표</p>
          </div>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <ComposedChart data={cumulative} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
            ...
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ fontSize: 12, fontFamily: FONT }} iconType="circle" iconSize={7} />
            <Line dataKey="cumGoal" name="누적 목표" type="monotone" stroke={BLUE} strokeWidth={2} dot={{ r: 3.5, fill: BLUE, stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 5 }} connectNulls />
