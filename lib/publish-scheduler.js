...
export function startPublishScheduler(opts = {}) {
  ...
  // 부팅 catch-up — 놓친 자정 보상. 120초 후: 연쇄 재배포 직후 트래픽·메모리가
  // 안정된 뒤에 돈다 (30초는 배포 직후 스파이크와 겹쳐 512MB 인스턴스에서 OOM 소지).
  const last = readUnifiedPublishMeta()
  if (missedRun(last?.ts, Date.now())) {
    log.info({ lastRun: last?.ts ? new Date(last.ts).toISOString() : null }, 'missed midnight run — catch-up in 120s')
