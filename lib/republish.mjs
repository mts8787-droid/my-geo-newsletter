...
export function assembleCitationData() {
  ...
  // citation 어드민의 사용자 편집 metaKo/metaEn 은 스냅샷에만 남는다 —
  // 스냅샷 meta 를 base 로, 시트 구조 필드(sync-data.meta)로 보강.
  const metaKo = { ...(snap?.metaKo || {}), ...(d.meta || {}), ...pickUserText(snap?.metaKo) }
  const metaEn = snap?.metaEn || {}
...
function publishDashboardChannels({ includeReadability }) {
  ...
  const combo = publishChannel(CHANNELS.dashboard, { htmlKo: comboKo, htmlEn: comboEn, title: comboTitle })
  ...
  const vis = publishChannel(CHANNELS.visibility, { htmlKo: visKo, htmlEn: visEn, title: visTitle })
