...
// ─── 시트별 원본 CSV 저장 — Raw 데이터 다운로드 탭용 (2026-09-02) ─────────────
// 매 동기화 때 시트(탭)별 gviz CSV 원문을 DATA_DIR/sheet-raw/ 에 저장.
// 메타 시트는 제외 (사용자 지시). index.json 이 대시보드 탭과 서빙 라우트의 목록 소스.
const SHEET_RAW_DIR = () => join(DATA_DIR, 'sheet-raw')
const RAW_EXCLUDED_SHEETS = ['meta', 'unlaunched', 'PR Topic List']   // 메타 + 언런치드 + PR 토픽 (사용자 지시 2026-09-02)
export function sheetRawSlug(name) {
  return String(name).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'sheet'
}
export function makeSheetRawCollector() {
  const raws = []
  const onRaw = (name, csv, rowCount) => {
    if (RAW_EXCLUDED_SHEETS.includes(name)) return
    raws.push({ name, csv: String(csv || ''), rows: rowCount || 0 })
  }
  return { raws, onRaw }
}
export function saveSheetRaw(raws) {
  if (!raws || !raws.length) return null
  const dir = SHEET_RAW_DIR()
  mkdirSync(dir, { recursive: true })
  const seen = new Set()
  const sheets = raws.map(r => {
    let slug = sheetRawSlug(r.name)
    while (seen.has(slug)) slug += '-2'   // 슬러그 충돌 방지 (이론상)
    seen.add(slug)
    writeFileSync(join(dir, `${slug}.csv`), r.csv)
    return { name: r.name, slug, rows: r.rows, bytes: Buffer.byteLength(r.csv) }
  })
  const index = { syncedAt: Date.now(), sheets }
  writeFileSync(join(dir, 'index.json'), JSON.stringify(index, null, 2))
  // 인덱스에 없는 잔존 csv 삭제 — 제외 목록이 바뀌어도 옛 파일이 직접 URL 로 남지 않게
  try {
    for (const f of readdirSync(dir)) {
      if (f.endsWith('.csv') && !seen.has(f.slice(0, -4))) unlinkSync(join(dir, f))
    }
  } catch (e) { log.warn({ err: e.message }, 'sheet-raw stale cleanup failed') }
  return index
}
export function readSheetRawIndex() {
  try { return JSON.parse(readFileSync(join(SHEET_RAW_DIR(), 'index.json'), 'utf-8')) } catch { return null }
}
...
export function updateSyncDataFromParsed(parsed) {
  ...
  // newsletter — 뉴스레터 어드민은 자체 sync-data(newsletter 모드)를 마운트마다 로드한다.
  // 통합 동기화가 이 파일을 안 쓰면 "시트 동기화를 해도 뉴스레터 사이테이션이 안 바뀌는"
  // 데이터 섬이 된다 (사용자 보고 2026-09-16). 시트 파생 키를 전부 갱신하되
  // 사용자 meta 텍스트와 얼려둔 readability 는 보존(...prevNl 스프레드).
  const prevNl = readModeSyncData('newsletter') || {}
  // 뉴스레터 발행 정보(호수·발행월·데이터 기준·제목)는 사용자 소관 — 시트 meta 로 덮지 않는다
  // (덮으면 어드민 접속 때마다 편집값이 되돌아가는 "고정" 증상, 2026-09-17)
  const nlSheetMeta = { ...(parsed.meta || {}) }
  ;['period', 'reportNo', 'dateLine', 'title'].forEach(k => { delete nlSheetMeta[k] })
  const nlNext = {
    ...prevNl,
    ...pickParsed(parsed, [...new Set([...DASH_SYNC_KEYS, ...CIT_SYNC_KEYS])]),
    meta: mergeMetaPreservingText(prevNl.meta, nlSheetMeta),
    savedAt: now,
  }
  if (tf) nlNext.total = dashNext.total
  writeModeSyncData('newsletter', nlNext)

  return { dash: dashNext, cit: citNext, nl: nlNext }
}
...
export function assembleCitationData() {
  ...
  // period 폴백 — citation 시트 meta 에 period 가 없으면 시트 파생값(citDerivedPeriod) 사용
  if (!metaKo.period && d.citDerivedPeriod) metaKo.period = d.citDerivedPeriod
  const metaEn = snap?.metaEn || {}
...
function publishDashboardChannels({ includeReadability }) {
  ...
  let combo, vis
  {
    ...
    const opts = { monthlyVis: a.monthlyVis, includeReadability, sheetRaw: readSheetRawIndex() }
...
function publishCitationChannel() {
  ...
  // EN = KO 구조(토글·period·fresh 설정) + 스냅샷의 EN 번역 텍스트만 오버레이.
  // metaEn 통째 사용은 KO 와 같은 낡은 스냅샷 오염 경로 (EN 제목 'Mar 2026' 회귀, 2026-09-02).
  const metaEn = Object.keys(c.metaEn).length ? mergeEnMeta(c.metaKo, c.metaEn) : c.metaKo
  // period 는 번역 텍스트가 아니라 데이터 파생값 — mergeEnMeta 의 번역 필드 목록에 들어 있어
  // 3월 스냅샷의 EN period 가 fresh 값을 덮었다 (EN 제목 'Mar 2026' 잔존, 2026-09-02)
  metaEn.period = c.metaKo.period
...
async function _runUnifiedPublish({ sheetUrl, trigger, sync }) {
  ...
  if (sync) {
    try {
      const { raws, onRaw } = makeSheetRawCollector()
      const parsed = await syncFromGoogleSheets(cfg.sheetId, undefined, { onRaw })
      updateSyncDataFromParsed(parsed)
      const rawIdx = saveSheetRaw(raws)   // 시트별 원본 CSV 저장 (메타 제외) — Raw 다운로드 탭
      result.syncIssues = parsed._syncIssues || []
      log.info({ trigger, keys: Object.keys(parsed).length, issues: result.syncIssues.length, rawSheets: rawIdx?.sheets.length || 0 }, 'sheet sync done')
    } catch (e) {
  ...
  // Progress Tracker 스냅샷 EN 사전 번역 셀프힐 — 번역 누락분만 채움 (없으면 no-op).
  // 게시본(비인증)이 번역 API 를 못 쓰는 구조라 여기서 채워 둔다 (2026-09-11).
  try {
    const { ensureTrackerI18nFile } = await import('./tracker-i18n.js')
    const r = await ensureTrackerI18nFile(join(DATA_DIR, 'tracker-v2-snapshot.json'))
    if (r.added) log.info({ added: r.added, total: r.total }, 'tracker i18n self-heal')
  } catch (e) {
    result.errors.push({ step: 'tracker-i18n', error: e.message })
    log.warn({ err: e.message }, 'tracker i18n self-heal failed')
  }
