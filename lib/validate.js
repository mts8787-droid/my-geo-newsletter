...
// 로컬 저장본 JSON 병합 import (POST /api/:mode/snapshots/import)
export const SnapshotImportSchema = z.object({
  ...
  })).min(1, 'snapshots 비어있음').max(100, 'snapshots 최대 100개'),
})
...
export const PublishPostSchema = z.object({
  ...
  // 월별 발행 채널 (newsletter) 용 — YYYY-MM
  month: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/, 'month 는 YYYY-MM 형식').optional(),
})
