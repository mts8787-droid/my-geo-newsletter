...
import { readFileSync, writeFileSync, mkdirSync, existsSync, renameSync, readdirSync, rmSync, openSync, readSync, closeSync } from 'fs'
...
// 대용량/손상 JSON 배열 파일을 항목 단위로 순회 (통파싱 없음).
// - 메모리: 항목 1개 크기로 제한 → 수백 MB 레거시 파일도 마이그레이션 중 OOM 없음
// - 복구: OOM-kill 중 잘린(truncated) 파일도 온전한 항목까지 전부 회수 (손상 지점에서 중단)
export function* iterJsonArrayItems(file, chunkSize = 1 << 22) {
  let fd
  try { fd = openSync(file, 'r') } catch { return }
  try {
    const buf = Buffer.alloc(chunkSize)
    const dec = new StringDecoder('utf8')  // 청크 경계 멀티바이트(한글) 안전
    let started = false, inStr = false, esc = false, depth = 0
    let cur = ''
    while (true) {
      const n = readSync(fd, buf, 0, chunkSize, null)
      if (n <= 0) break
      const text = dec.write(buf.subarray(0, n))
      let segStart = -1  // 현재 청크 내 요소 시작(또는 이어짐) 위치
      if (depth > 0) segStart = 0
      for (let i = 0; i < text.length; i++) {
        const c = text[i]
        if (!started) { if (c === '[') started = true; continue }
        if (depth === 0) {
          if (c === '{') { depth = 1; segStart = i; inStr = false; esc = false }
          else if (c === ']') return  // 배열 정상 종료
          continue
        }
        if (inStr) {
          if (esc) esc = false
        ...
        if (c === '"') inStr = true
          ...
          depth--
          if (depth === 0) {
            cur += text.slice(segStart, i + 1)
            segStart = -1
            try { yield JSON.parse(cur) } catch { return }  // 손상 지점 도달 → 회수분까지만
            cur = ''
          }
        }
      }
      if (depth > 0 && segStart >= 0) cur += text.slice(segStart)
      if (cur.length > 256 * 1024 * 1024) return  // 비정상 초대형 단일 항목 방어
    }
...
function itemStore(dirName, legacyFile) {
  ...
  let healed = false  // 프로세스 내 self-heal 1회 가드
  // src 파일에서 항목을 스트리밍 추출 → per-file + 인덱스 재작성
  function migrateFrom(src, renameAfter) {
    const idx = []
    if (src && existsSync(src)) {
      for (const s of iterJsonArrayItems(src)) {
    ...
    if (renameAfter && src && existsSync(src)) renameSync(src, `${src}.migrated`)
    if (idx.length) console.log(`[storage] ${dirName}: 레거시 ${idx.length}건 → 파일 분리 마이그레이션 완료`)
    return idx
  }
  function ensure() {
    if (existsSync(indexFile)) {
      // self-heal: 인덱스가 비어있는데 .migrated(레거시 원본)가 남아있으면 —
      // 이전 통파싱 마이그레이션이 손상/OOM 으로 빈 결과를 남긴 케이스 → 스트리밍 복구 재시도
      if (!healed && legacyFile && existsSync(`${legacyFile}.migrated`)) {
        healed = true
        let empty = true
        try { empty = JSON.parse(readFileSync(indexFile, 'utf-8')).length === 0 } catch { empty = true }
        if (empty) {
          const idx = migrateFrom(`${legacyFile}.migrated`, false)
          if (idx.length) console.log(`[storage] ${dirName}: self-heal — .migrated 에서 ${idx.length}건 복구`)
        }
      }
      return
    }
    mkdirSync(dir, { recursive: true })
    migrateFrom(legacyFile, true)
  }
  function list() {
    ensure()
