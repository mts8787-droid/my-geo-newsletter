...
```
세션 시작 (startup) · /clear · compact 시  (※ --resume 세션은 매처 제외 — 원 컨텍스트가 복원되므로)
...
- **엔진 = using-hiro** (`.claude/skills/using-hiro/SKILL.md`): 디스패치 맵·red-flags·비협상·완료 검증. 세션 시작(startup/clear/compact)마다 훅으로 주입되므로 스킬이 상황에 맞춰 **자동 발동**한다.
