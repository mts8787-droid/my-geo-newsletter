---
name: onboard
description: 새 프로젝트에 본 하네스를 적용할 때 사용. 사용자가 "이 하네스 적용해줘" / "내 프로젝트에 셋업해줘" 같이 요청하면 본 스킬 사용. 도메인 인터뷰 → 도메인 파일 생성 → 데이터 모델 → 디자인 토큰 → 외부 시스템 → 빌드 검증 8 step 워크플로우.
---

# Onboard — 새 프로젝트 하네스 적용 워크플로우

> 사용자가 ZIP 받아서 다른 프로젝트에 적용 요청 시 본 스킬 사용.
> 룰 매뉴얼: `.claude/rules/BOOTSTRAP.md` 참조.
...
```
1. 사전 확인:
   · CLAUDE.md / .claude/ / docs/agents/ 디렉토리 모두 프로젝트 루트에 있나
   · chmod +x .claude/hooks/*.sh 실행됐나
   · npm install / package.json 확인

2. 도메인 인터뷰 — 사용자에게 6 질문 (한 번에 또는 단계별):
   a) "프로젝트 도메인은? (예: 매출 / HR / 제품 사용량 / 마케팅 KPI / ...)"
...
3. 도메인 파일 (categoryMap.js 또는 동등) 생성:
   · src/categoryMap.js 신설 (또는 변경)
   · CATEGORY_IDS (또는 도메인 적절 이름) 배열
...
6. 외부 시스템 — .env:
   ...
   · AI 사용 시 ANTHROPIC_API_KEY 등재 + ai.md 룰 자동 적용

7. 비즈니스 fact 등록:
   · 사용자에게 "데이터 무관 항상 적용 룰이 있나요?" 질문
...
기존 사용자가 적용한 하네스를 본인 도메인에 맞게 추상화 / 일반화 필요 시.

```
1. CLAUDE.md NEVER 룰 검토 — 사용자 도메인에 맞는지
2. docs/agents/ 의 미러 문서 — "LG", "Samsung", "GEO Visibility" 같은 본 레포
   도메인 용어가 사용자 프로젝트에 들어가지 않게
3. .claude/rules/ 의 토큰/invariant 가 일반화되어 있는지 (도메인 추상)
4. .claude/skills/ 의 워크플로우가 사용자 도메인에 맞게 step 조정 가능한지
...
본 레포의 하네스가 잘 정착된 후, 추가로 룰/훅/스킬을 보강할 때.

```
1. 발견: 코드 회귀 / 반복 실수 / 비효율 패턴 인지
2. 분류:
   · "따라야 할 규칙" → 룰 (.claude/rules/ 적절 파일)
   · "절대 하면 안 되는 것" → 훅 (.claude/hooks/ 스크립트 + settings.json 등록)
   · "순차 워크플로우" → 스킬 (.claude/skills/<name>/SKILL.md)
   · "특정 영역 분리 작업" → 서브에이전트 (.claude/agents/<name>.md)
