---
name: onboard
description: 새 프로젝트에 본 하네스를 적용할 때 사용. 사용자가 "이 하네스 적용해줘" / "내 프로젝트에 셋업해줘" 같이 요청하면 본 Skill 사용. 도메인 인터뷰 → 도메인 파일 생성 → 데이터 모델 → 디자인 토큰 → 외부 시스템 → 빌드 검증 8 step 워크플로우.
---
...
```
1. CLAUDE.md NEVER Rule 검토 — 사용자 도메인에 맞는지
2. docs/agents/ 의 미러 문서 — "LG", "Samsung", "GEO Visibility" 같은 본 저장소
