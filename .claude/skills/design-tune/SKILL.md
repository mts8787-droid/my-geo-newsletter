---
name: design-tune
description: '디자인 미세 조정·호환·회귀 디버깅 (이메일 호환 table-layout 변환, KO/EN 라벨 추가, UI 깨짐·정렬 회귀 수정, 픽셀 단위 조정). 기존 UI 깨짐·호환성 문제·작은 조정이 필요할 때 호출 (새 컴포넌트는 design-component). 트리거: "카드 깨졌어" / "이메일에서 차트 안 보임" / "다국어 라벨 (KO/EN) 추가" / "폰트 1px 키워줘" 같은 요청.'
---
