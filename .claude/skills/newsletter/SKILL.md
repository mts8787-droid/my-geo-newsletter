---
name: newsletter
description: 뉴스레터 / 월간·주간 리포트 / 이메일 호환 HTML 작업 워크플로우. 신규 뉴스레터 발행본 작성, 새 섹션·카드 추가, 발송 전 다중 클라이언트 호환 검증 (Outlook/Gmail/Apple Mail), 미출시 국가 회색 처리, KO/EN 동시 생성, SMTP 발송. 사용자가 "뉴스레터 만들어줘", "월간 리포트 새 섹션", "이메일 발송", "preheader 추가" 같이 요청 시 트리거. Rule·invariant·ANTI-PATTERN 은 .claude/rules/newsletter.md 참조.
---
