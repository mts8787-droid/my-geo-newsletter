...
description: 시각 컴포넌트 추가 워크플로우 — 신규 카드/차트/테이블(C-XX) 컴포넌트 추가, 뉴스레터 제품 카드 변형(V1/V2/V3 → V4) 추가, 어드민 페이지에 iframe srcdoc 미리보기 추가. "새 카드 만들어줘", "C-24 추가", "뉴스레터 V4" 같이 신규 시각 단위 추가 시 트리거. 디자인 토큰·컴포넌트 카탈로그는 .claude/rules/design.md 참조.
---
