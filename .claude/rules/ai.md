...
본 Rule 은 **Anthropic API 호출 + 응답 검증 + 비용 추적** 의 공통 패턴. 본 저장소는 가전 KPI 인사이트 생성이지만, **AI 호출이 필요한 모든 도메인** 에 동일 패턴.
...
- **고객 지원 챗봇** — 응답 검증 (빈/거절) + 비용 추적 + 사용자 메시지 보호
- **문서 요약** — 긴 입력에 streaming + 프롬프트 캐싱
- **번역** — 사용자 원문 그대로 (다듬기 금지) + 응답 검증
- **코드 리뷰** — 응답 검증 + 호출 로그 + 비용 모니터링
