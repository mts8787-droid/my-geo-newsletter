...
> 실제 작업 워크플로우 (step-by-step) 는 `.claude/skills/design/SKILL.md` (Skill) 참조.
...
**폰트 파일 로딩 (`@font-face`) — 독립 서버 렌더 HTML 필수**:
- `font-family` 토큰(`FONT`)은 **family 이름만** 나열. 실제 `.ttf/.otf` 를 불러오려면 페이지 `<style>` 에 `@font-face` 규칙(→ `/font/<file>`)이 **반드시 임베드**되어야 함. 없으면 시스템 폰트로 silent 폴백 (한글 LG Smart / 영문 LGEIText·Arial Narrow 미적용).
- `/font` 정적 경로는 `routes/spa-static.js` 가 서빙. URL 은 절대경로 (`/font/...`) → 어느 페이지 경로에서도 해석됨.
- 필수 블록 (weekly/citation/monthly/email/dashboard·render-readability 공통):
  - **LGEIText** 4 weight (100–300 / 400–500 / 600 / 700–900) — `%20` 인코딩 URL + `font-display:swap`
  - **LG Smart** 4 weight (300 / 400 / 600 / 700) — 한글
  - **LG Smart CDN** `<link href="https://fonts.cdnfonts.com/css/lg-smart">` (옵션 — 로컬 파일 보강)
  - 본문 `<style>` 와 **빈/폴백 `<style>` 양쪽** 모두에 넣을 것 (스냅샷 없음 화면도 폰트 적용).
- 참조 구현: `src/weekly-report/weeklyTemplate.js` (LGEIText + LG Smart 둘 다 선언 — 가장 완전한 패턴).
- ⚠ **공통 헬퍼 없음** — 현재 각 서버 렌더 페이지가 인라인 중복. 신규 독립 HTML 생성기 추가 시 이 블록 복사 누락 주의 (실제 회귀: render-readability.mjs 가 통일 작업 때 `@font-face` 누락 → 폰트 미적용).
...
### 5.11 라인 차트 양식 7종 (Chart.js 표준 매핑 + 본 저장소 SVG 패턴)

Chart.js 의 `docs/samples/line` 디렉토리 기준 7가지 양식. 본 저장소의 `dashboardSvg.js` 의 svgLine / svgMultiLine 으로 구현 시 가이드.

#### 1) Basic Line (`line`)
가장 단순한 직선 라인. **본 저장소 기본** — `svgLine(data, labels, w, h, color, opts={})`.
...
- SVG 구현: `Q` (quadratic Bezier) 또는 `C` (cubic Bezier) path command
  ...
  ```
- **본 저장소 RULE**: 시각 일관성 위해 트렌드 차트 **직선만 사용** (Chart.js 가 보여줘도 본 저장소는 단순한 직선이 KPI 변화 추적에 더 정확). 곡선 적용 시 시각 왜곡 가능 (보간된 점이 실제 값처럼 보임).
...
- SVG 구현: viewBox 좌측 = primary Y축 (%), 우측 = secondary Y축 (count)
  ...
  ```
- **본 저장소 현재**: 단일 Y축만. 신규 차트가 단위 섞인 경우 svgMultiLine 의 옵션으로 확장 권장.
...
- 추가 패턴 (필요 시):
  ...
  ```
- **본 저장소 RULE**: 점 모양 일관 — 한 차트에 다른 모양 섞기 X. **베이스라인 시작점만 예외**.

#### 5) Segments (`segments`)
한 라인을 구간별로 다른 색·두께·dash 적용.
- **본 저장소 패턴**: `fadeBeforeIdx` 가 이 컨셉 — pre = 회색 (`#64748B`, opacity 0.85) + post = 컬러
  - svgLine / svgMultiLine 둘 다 지원
- SVG 구현: 두 path 로 분리 (§5.3 [1] Bridge 제거 Rule)
  ```svg
...
- 또는 `H` (horizontal) / `V` (vertical) command:
  ...
  ```
- **본 저장소 현재 사용 X** — 현재 KPI 는 연속 측정. 정책·시점 변경 같은 step-change 데이터면 적합.
...
### 5.12 다른 차트 양식 5종 (Chart.js 표준 + 본 저장소 SVG 매핑)

Chart.js 의 `docs/samples/other-charts/` 5 종. 본 저장소는 일부 사용 중·일부 미사용. 미사용 양식도 도입 후보 명시.
...
- **SVG 구현**:
  ...
  ```
- **본 저장소 유사**: 4분면 점도표 (C-15) — 반지름 고정 (단순화 bubble). 진짜 bubble (가변 r) 미사용.
...
- **SVG 구현**:
  ...
  ```
- **본 저장소 미사용** — 비중 표시는 vbar (`_citVBar`) 사용 중.
- **도입 후보**: 어드민 페이지의 인사이트 호출 type 분포 (observability) / dotcom 카테고리별 비중.
- **Pie vs Doughnut**: cutout 만 차이 (40~60% 권장). 라벨 가운데 표시 가능한 doughnut 이 본 저장소 디자인에 더 적합.
...
**왜 쓰나?** Skill이 차트를 그릴 때 어떤 양식인지 명확히 지정 — `"L-1 형태로 라인 차트 만들어줘"` 같이 코드만 알려주면 정확한 양식으로 그려짐.
...
- 라이브 페이지: `/admin/chart-library` — 21 카드 (Chart.js 15 + 본 저장소 6) + 카테고리 색상 라벨
- 정적 미러: `harness-mirror/docs/CHART_LIBRARY.html` — 브라우저 더블클릭, 인증 없이 열람
- Skill: `.claude/skills/design/SKILL.md` 의 "차트 그리기 (분류 코드)" 워크플로우

#### 사용 명령 예시

- `"L-1 차트 그려줘"` → Basic Line 양식
- `"V-1 그룹 막대"` → 본 저장소 vbar
- `"BP-1 범프"` → 순위 변화 차트
- `"M-1 + T-1"` → 콤보 + 툴팁
- `"D-2 파이"` → 비중 시각화
...
3. 본 §5.15 의 두 표 (Chart.js 표준 / 본 저장소 실제) 에 등재
...
- 정적 미러: `harness-mirror/docs/CHART_LIBRARY.html` — 브라우저 더블클릭, 인증 없이 열람
- Skill: `.claude/skills/design/SKILL.md` 의 "차트 그리기 (분류 코드)" 워크플로우 (10 step)
...
**본 저장소 적용 패턴** (서버 SVG 차트 + 클라이언트 hover):
...
NEVER  독립 서버 렌더 HTML 에 FONT 토큰만 쓰고 @font-face 블록 누락 → family 이름만 있고 폰트 파일 미로드 → 시스템 폰트 silent 폴백 (한글 LG Smart / 영문 LGEIText·Arial Narrow 미적용). 본문 + 빈/폴백 <style> 양쪽에 @font-face(LGEIText 4 + LG Smart 4) 임베드 (§4.4 "폰트 파일 로딩"). 실제 회귀: render-readability.mjs
```

### 6.1 Template Literal 보간 함정

`generateXxxHTML` 같은 서버 측 HTML 생성기는 거대한 template literal (return \`…\`) 안에 inline `<script>` 를 포함. 그 inline JS 의 **주석 안 `${var}`** 도 외부 template 이 평가하려고 시도 → 외부 스코프에 변수 없으면 `ReferenceError` → React useMemo throw → 빈 화면.

**회피**: 주석에 placeholder 표기 (`<var>` / `[var]`) 사용.
...
**확정 회귀** (1건): `citationTemplate.js:1372` (commit `aaa89b5` → `ff80f4b` 수정) — `// 키 형식: '${cnty}|${k}'` → `cnty is not defined`.
