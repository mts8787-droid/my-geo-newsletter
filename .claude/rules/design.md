...
본 Rule 은 디자인 시스템의 **공통 패턴**. 본 저장소는 가전 KPI 대시보드 + 뉴스레터라 색상·컴포넌트·차트가 가전 도메인 색깔이지만, **패턴 자체는 어떤 데이터 시각화에든 적용**.
...
- **매출 대시보드** — `BRAND_COLORS` 대신 `CHANNEL_COLORS` / `STATUS` 그대로 사용
- **HR 대시보드** — `BRAND_COLORS` → `DEPT_COLORS` / 베이스라인 마커는 정책 변경 시점 표시
- 가전 도메인 외에 — 차트 분류 코드 (`L-1`, `BP-1` 등) 는 그대로 호출 가능
