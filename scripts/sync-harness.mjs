...
.sub{font-size:16px;color:var(--text-muted);margin-bottom:20px}
...
table{width:100%;border-collapse:collapse;font-size:16px;margin-bottom:18px;background:var(--bg-card);border-radius:8px;overflow:hidden}
th{text-align:left;color:var(--text-sub);font-weight:600;padding:10px 14px;border-bottom:1px solid var(--border);font-size:14px;text-transform:uppercase;letter-spacing:1px}
...
.section ul{margin:6px 0 12px 22px;font-size:16px;color:var(--text-desc)}
.section li{margin-bottom:4px}
.tree{font-family:ui-monospace,Menlo,Consolas,monospace;font-size:15px;background:var(--bg-code);padding:14px 18px;border-radius:8px;color:var(--text-desc);white-space:pre;margin-bottom:14px;line-height:1.4}
...
.note{background:#0F2A1F;border:1px solid #15803D;border-radius:8px;padding:12px 16px;margin-top:14px;font-size:15px;color:#86EFAC}
...
.warn{background:#3F1D1D;border:1px solid #7F1D1D;border-radius:8px;padding:12px 16px;margin-top:14px;font-size:15px;color:#FCA5A5}
...
a{color:var(--accent)}
.back-link{display:inline-block;color:var(--accent);text-decoration:none;font-size:16px;margin-bottom:14px;font-weight:600}
...
@media (max-width:780px){
  ...
  .tree{font-size:14px;padding:12px 14px;overflow-x:auto}
  table{font-size:15px}
  th{padding:8px 10px;font-size:13px;letter-spacing:0.6px}
  td{padding:8px 10px}
...
@media (max-width:480px){
  ...
  .note,.warn{font-size:14px;padding:10px 12px}
  .back-link{font-size:15px;margin-bottom:10px}
}
</style></head><body>
${themeToggleButton()}
...
<div class="section">
  ...
  <ol style="margin-left:22px;font-size:16px;color:var(--text-desc)">
    ...
    <li>
      <strong>Hook 실행 권한 부여</strong> — Hook이 실행되려면 필수. 터미널에서 <strong>압축을 풀었던 프로젝트 루트 폴더로 이동</strong> (<code>cd /your/project/path</code>) 한 다음 한 줄 실행:
