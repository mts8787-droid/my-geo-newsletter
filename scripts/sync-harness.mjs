...
// 원본 하네스 파일 (CLAUDE.md, docs/*_RULES.md, .claude/skills/*/SKILL.md, .claude/hooks/*.sh)
// → .claude/agents/ 미러 폴더 자동 동기화.
...
const MIRROR_MAP = [
  // 헌법
  { src: 'CLAUDE.md', dst: '.claude/agents/CLAUDE.md' },
  // 룰 매뉴얼
  { src: 'docs/DATA_RULES.md',   dst: '.claude/agents/rules/data.md' },
  { src: 'docs/DESIGN_RULES.md', dst: '.claude/agents/rules/design.md' },
  // 스킬 워크플로우 (frontmatter 제거 — 미러는 참조용)
  { src: '.claude/skills/data/SKILL.md',   dst: '.claude/agents/skills/data.md',   stripFrontmatter: true },
  { src: '.claude/skills/design/SKILL.md', dst: '.claude/agents/skills/design.md', stripFrontmatter: true },
]
...
- **warn-silent-fallback.sh** (PostToolUse): \`return {}\` 단독 라인 검출 → \`_logWarn\` 사용 권고. \`docs/DATA_RULES.md\` ANTI-PATTERN 강제.
- **block-categorymap-bypass.sh**: \`src/categoryMap.js\` 외 파일에 \`PROD_ID_TO_\` 같은 정의 추가 차단.

## 관련 룰 / 스킬
- 룰: \`docs/DATA_RULES.md\` (5단계 ERROR CATCHING · ANTI-PATTERN)
...
fs.mkdirSync(path.join(ROOT, '.claude/agents/hooks'), { recursive: true })
fs.writeFileSync(path.join(ROOT, '.claude/agents/hooks/data.md'),   HOOKS_DATA_MD,   'utf8')
fs.writeFileSync(path.join(ROOT, '.claude/agents/hooks/design.md'), HOOKS_DESIGN_MD, 'utf8')
...
fs.writeFileSync(path.join(ROOT, '.claude/agents/HARNESS.md'), HARNESS_MD, 'utf8')
...
<div class="intro">
  <p><strong>본 폴더 <code>.claude/agents/</code> 는 본 레포의 모든 하네스 컴포넌트의 미러링 본을 모은 곳.</strong> 정보 구조 단순화 — 여기 한 곳만 보면 전체 파악 가능.</p>
  <p>원본 파일이 수정되면 <code>npm run sync:harness</code> 또는 <code>npm run build</code> (prebuild) 시 자동 갱신.</p>
</div>
...
<div class="section">
  ...
  <p>원본: <code>CLAUDE.md</code>, <code>docs/DATA_RULES.md</code>, <code>docs/DESIGN_RULES.md</code></p>
</div>
...
<div class="section">
  <h2>본 레포 갱신 흐름</h2>
  <ol style="margin-left:22px;font-size:13px;color:#CBD5E1">
    <li>원본 파일 수정 (예: <code>docs/DATA_RULES.md</code>, <code>.claude/skills/data/SKILL.md</code>)</li>
...
fs.writeFileSync(path.join(ROOT, '.claude/agents/HARNESS.html'), HARNESS_HTML, 'utf8')
